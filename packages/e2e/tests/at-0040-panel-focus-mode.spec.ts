...
test.describe("AT-0040 Panel focus mode", () => {
  test("collapses and expands the sidebar in ProjectMode (TC-1)", async ({ page }) => {
    await page.goto("/");
  ...
  test("enters and exits preview focus mode (TC-2)", async ({ page }) => {
    await page.goto("/");
  ...
  test("preserves sidebar-collapsed state across focus toggle (TC-5)", async ({ page }) => {
    await page.goto("/");
