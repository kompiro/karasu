...
import {
  ...
  displayRootFor,
  findingKeys,
  type DiagnosticDocumentContext,
} from "./diagnostic-location.js";
...
// The components pass the locale's `Line N`; a fixed spelling stands in here.
const label = (
  loc: Parameters<typeof diagnosticLocationLabel>[0],
  ctx: DiagnosticDocumentContext,
) => diagnosticLocationLabel(loc, ctx, (line) => `Line ${line}`);
...
describe("diagnosticLocationLabel (#2715)", () => {
  it("has nothing to show for a diagnostic without a position", () => {
    expect(label(undefined, context)).toBeNull();
  });
  ...
  it("reads a position without a file as the open document's", () => {
    expect(label(at(4), context)).toBe("Line 4");
  });

  it("keeps the line form for the open document's own positions", () => {
    expect(label(at(4, "/projects/shop/index.krs"), context)).toBe("Line 4");
  });

  it("names an imported file relative to the display root", () => {
    expect(label(at(12, "/projects/shop/slices/legacy.krs"), context)).toBe("slices/legacy.krs:12");
  });
  ...
  it("names the entry when another file is the one open", () => {
    ...
    expect(label(at(2, "/projects/shop/index.krs"), legacyOpen)).toBe("index.krs:2");
  });

  it("tolerates a trailing slash on the display root", () => {
    const slashed = { ...context, displayRoot: "/projects/shop/" };

    expect(label(at(7, "/projects/shop/theme.krs.style"), slashed)).toBe("theme.krs.style:7");
  });
  ...
  it("shows the full path when the file is not under the display root", () => {
    expect(label(at(1, "/elsewhere/shared.krs"), context)).toBe("/elsewhere/shared.krs:1");
    expect(
      label(at(1, "/elsewhere/shared.krs"), {
  ...
  it("names a compared snapshot's file by its project-relative path", () => {
    expect(label(at(4, "/.snapshot-view/snap-1/index.krs"), context)).toBe("index.krs:4");
    expect(label(at(9, "/.snapshot-view/snap-1/slices/legacy.krs"), context)).toBe(
      "slices/legacy.krs:9",
    );
  });
  ...
  it("does not shorten a sibling directory that merely shares the root's prefix", () => {
    expect(label(at(1, "/projects/shopping/index.krs"), context)).toBe(
