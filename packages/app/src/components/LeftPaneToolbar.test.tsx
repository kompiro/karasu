...
describe("LeftPaneToolbar", () => {
  it("renders Format button on editor tab when onFormat is provided", () => {
    const { getByRole } = render(
      <LeftPaneToolbar activeTab="editor" onFormat={vi.fn()} />,
    );
  ...
  it("renders nothing on chat tab", () => {
    const { queryByRole } = render(
      <LeftPaneToolbar activeTab="chat" onFormat={vi.fn()} />,
    );
  ...
  it("renders nothing on settings tab", () => {
    const { queryByRole } = render(
      <LeftPaneToolbar activeTab="settings" onFormat={vi.fn()} />,
    );
    expect(queryByRole("button")).toBeNull();
  });
  ...
  it("Format button is disabled when hasParseErrors is true", () => {
    const { getByRole } = render(
      <LeftPaneToolbar activeTab="editor" onFormat={vi.fn()} hasParseErrors />,
    );
  ...
  it("Format button is enabled when hasParseErrors is false", () => {
    const { getByRole } = render(
      <LeftPaneToolbar activeTab="editor" onFormat={vi.fn()} hasParseErrors={false} />,
    );
  ...
  it("clicking Format button calls onFormat", () => {
    ...
    const { getByRole } = render(
      <LeftPaneToolbar activeTab="editor" onFormat={onFormat} />,
    );
