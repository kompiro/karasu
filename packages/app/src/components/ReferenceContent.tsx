...
// The reference content (syntax snippets, selector examples, the specificity
// table) is sourced entirely from core's `getReference()` payload — moved out
// of this component so the snippets can't drift from `docs/spec/*.md` (#1586).
...
function StylesTab({ activeView }: { activeView: ActiveView }) {
  ...
  return (
    <div className="reference-tab-body">
      ...
      <div className="reference-code-block">
        <pre>{ref.styleSelectorExamplesByView[refViewOf(activeView)]}</pre>
      </div>
      ...
      <table className="reference-table">
        ...
        <tbody>
          {ref.selectorSpecificity.map((row) => (
            <tr key={row.label + row.example}>
              <td>{row.label}</td>
              <td>
