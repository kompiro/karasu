...
# ADR-1642: example を examples/<lang>/<name>/ に揃え、docs gallery を en/ja 完全対応にする（アプリは最小シード）
...
- **関連**:
  ...
  - [ADR-1575](./1575-docs-site-ssg.md)（docs-site / gallery）、[ADR-1574](./1574-guide-embedded-diagrams.md)（guide diagrams）、[ADR-813](./813-i18n-default-policy.md)（i18n default policy）
...
- **正典は `examples/`**: gallery は `examples/` を single source of truth にビルド時レンダリング（ADR-1575）。`examples.ts` 同梱内容は移動後も on-disk と byte 一致を維持（`examples-sync` + `examples.test.ts` の drift ガード）。
