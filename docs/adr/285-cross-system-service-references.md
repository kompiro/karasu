...
# ADR-285: クロスシステムサービス参照 — ドット記法（`SystemId.ServiceId`）

- **日付**: 2026-04-04
- **ステータス**: 決定済み
- **関連**: Issue #285, [ADR-281](281-wildcard-import-two-pass-resolution.md), [ADR-328](328-ghost-system-rendering.md)
...
本 ADR のスコープは**構文導入のみ**。クロスシステム参照のレンダリング（ghost system ボックス、`ViewSlice.ghostSystems` / `ghostSystemEdges` フィールド追加、サブレイアウト対応）は [ADR-328](328-ghost-system-rendering.md)（Issue #328）で扱う。
