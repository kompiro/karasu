...
date: 2026-05-11
topic: edges
related_to: [ADR-968, ADR-1096, ADR-9019, ADR-1184]
assumptions:
...
- **関連**:
  ...
  - 関連 ADR:
    - [ADR-968](968-orthogonal-edge-routing-skip-layer.md) — 直交チャネルルーティング（パイプライン順序の前段）
    - [ADR-1184](1184-edge-label-position-offset.md) — `label-position` / `label-offset` （author override が本 ADR の slide に優先する）
...
2. `distributePorts`（ADR-968 系の Phase 3）が同一 `(node, side)` の port を
   `i/(N+1)` で分散
...
- `renderEdge` は `style.labelPosition === 0.5`（author が `label-position` を未設定）
  ...
  にスライドする。author override がある場合は ADR-1184 の挙動を優先
...
- **author override の自然な共存**: `style.labelPosition` の閾値判定 (`=== 0.5`) を
  switch にすることで、ADR-1184 で導入したスタイル制御が本 ADR の自動 slide を
  常に上書きできる。後方互換が破れない
...
edge id selector（ADR-1096）や edge direction style（ADR-9019）が edge 単位で
