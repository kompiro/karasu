...
# ADR-351: resource shape 自動推論とインフラノード Icon Mode 対応

- **日付**: 2026-04-07
- **ステータス**: 決定済み
- **関連**: Issue #351, [ADR-316](316-database-as-first-class-node.md), [ADR-30](30-icon-mode.md)

## 背景

ADR-316 (#351) で `database` / `queue` / `storage` が System 図にファーストクラスノードとして登場し、`resource OrderDB.OrderTable` のようなドット記法参照が UseCase 図等に現れるようになった。これに伴い 2 つの課題が残っていた：
