...
# ADR-464: `karasu apply` サブコマンド — stdin + `applyKrsPatch` を core に移動

- **日付**: 2026-04-11
- **ステータス**: 決定済み
- **関連**: Issue #464, Issue #447, [ADR-355](355-cli-translate-command.md), [ADR-469](469-cli-mutation-subcommands.md)
...
- **stdin 経由**: 引数の数による挙動切替は直感的でない。`apply` と `append` / `insert`（ADR-469）の一貫したインターフェースになる
...
- `karasu remove <node-id> <krs-file>` → ADR-469
- `karasu append <krs-file>`（stdin） → ADR-469
- `karasu insert <parent-id> <krs-file>`（stdin） → ADR-469
