...
# ADR-1729: app E2E（Playwright）はラベル駆動をやめ path filter で起動する
...
付いているときだけ実行される（ADR-529 の「保守コスト抑制」のための
opt-in 設計）。
...
- トレードオフ: `paths` のホワイトリスト漏れで「本当は E2E したい PR に
  ...
  小さい（ADR-579 と同じ評価）。nightly が最終セーフティネットとして
  残る。
- ADR-529 の三層 QA モデル・AI 視覚レビュー・パイロットスコープ等の
...
- ~~`vscode-e2e.yml`（`vscode-e2e` / `vscode-webview-e2e` ラベル駆動）は同じ
  ...
  評価する。~~ → **[ADR-1742](1742-vscode-e2e-path-filter-trigger.md)**
  で同型の path filter 移行を実施した（flake トレードオフの評価込み）。

## 将来の読者への注意

- もし将来「ラベルを付けたときに E2E をトリガしたい」ニーズが再発したら、
  ADR-579 の注意に従い、ラベル反応は **別 workflow + 別 concurrency
