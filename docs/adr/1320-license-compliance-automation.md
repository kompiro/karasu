...
# ADR-1320: OSS リリースのライセンス順守を allowlist CI と自動生成 THIRD_PARTY_NOTICES で機械化する
...
- **関連**:
  ...
  - 関連 ADR: [ADR-1315](1315-release-automation-changesets.md)（changesets による publish 配線 — 本件で生成される `THIRD_PARTY_NOTICES.md` を `npm publish` / `vsce package` 成果物に含める前提）
...
- **`@karasu-tools/*` の workspace 内部パッケージを除外したのは、karasu repo 自身の `LICENSE` でカバーされ二重記載になるから**。`@karasu-tools/core` は ADR-1315 で CLI に esbuild バンドルされるが npm 依存ゼロのため transitively 漏れる第三者ライセンスはない（将来 core に prod 依存を入れる場合は再確認）。
