...
# ADR-1142: GUI 駆動の `.krs.style` 編集 — 単一プロパティ rule は in-place 更新、その他は append fallback
...
ADR-1076 は GUI 編集を **append-only** と決めた。当時の主な根拠は:

1. Edge を一意に指す ID が無く、「同じ rule に対する更新」を判定する手段が無かった
2. 整形・コメントを保つ AST writer の作り込みが重いと見積もった

ADR-1096 で `edge#<canonicalId>` セレクタが定義・実装され、(1) は
...
### 案: append-only のまま、`Tidy Style` コマンドで後から整理する（旧 ADR-1076 の方針）
