...
# ADR-1411: App キーボードショートカットはコマンドレジストリを基盤にする
...
| `mod+shift+1` | `editView.showEditor` | エディットペインのタブを Editor に切替 | #1462 |
| `mod+shift+2` | `editView.showChat` | エディットペインのタブを Chat に切替 | #1462 |
...
`mod+shift+1` / `mod+shift+2` の `editView.showEditor` / `editView.showChat`
はエディットペインの `EditTabBar`（Editor / Chat / Settings）のタブを切替える。
ダイアグラムビューの `mod+1..4` と別系統であることを示すため `shift` 付きの
数字列とした。Settings（`mod+shift+3`）は設定画面であり主要な作業タブでは
ないため、意図的に未割り当てとしている。両コマンドは `whenTextInputFocused:
"allow"` — Editor から切替えるときは Monaco エディタ（`<textarea>`）に、Chat
から Editor に切替えるときは Chat 入力欄にフォーカスがあるのが通常で、`skip`
だと「最も切替えたい場面」で死ぬため。`command.openCommandPalette` に続く
2 つ目の `allow` 系コマンドである。

数字列の chord を `shift` 付きで使うには chord 正規化の修正が必要だった。
`eventToChord` は従来 `event.key` から chord を組み立てていたが、US 配列では
