...
# ADR-1184: edge `label-position` / `label-offset` プロパティ — fractional 位置と screen-axis CSS shorthand offset

- **日付**: 2026-05-09
- **ステータス**: 決定済み
- **関連**:
  - 親 Issue [#1184](https://github.com/kompiro/karasu/issues/1184)（親 #1071 — 既に close）
  - 関連 ADR: [ADR-9019](./9019-edge-direction-style.md)（同じ append-only `.krs.style` プロパティのパターンを踏襲）
...
`direction` プロパティ（ADR-9019）と同じく、**`.krs.style` で
...
- **`direction` と同じ append-only / cascade-tail-wins パターン**: GUI
  編集器（ADR-1076）にも素直に乗る。将来 right-click メニューに
...
- **invalid silent fallback**: warning を増やさない。ADR-9019 の
  `direction` と同様の方針
...
- **GUI 右クリックメニュー対応**: `Label position ▸ Start / Middle / End`
  の追加。`direction` 統合（ADR-1076）と同じパターンで実装可能
