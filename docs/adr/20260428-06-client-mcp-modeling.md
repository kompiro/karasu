...
### Feature toggle policy との関係（ADR-20260427-01）

ADR-20260427-01 では feature toggle を「UI のエントリポイントを隠すための短寿命のコンパイル時定数」と定義し、**パーサー拡張・レンダラー・ストア等は原則として常時ビルドし常時テストする**と明記している。
...
構文追加そのものではなく、`client` を活用する **UI 側のエントリポイント**（例: client 専用 detail panel、ノード storage 一覧表示など）を MVP 後に追加するときは、ADR-20260427-01 に従って通常の feature toggle で囲んで開発・卒業する。
これは本 MVP のスコープ外。
