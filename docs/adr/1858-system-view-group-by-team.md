...
date: 2026-07-11
topic: renderer
related_to: [ADR-1724, ADR-1821, ADR-1983]
scope:
  concerns: []
---

# ADR-1858: system view を team（owns）軸でグループ化し、折り畳み可能な境界フレームで密度を下げる
...
1. **メンバー範囲 = 全ノード種**（著者意図優先）。group 宣言されたノードは kind を問わず枠内。Group by ビュー内では **group 所属 > category（kind/tag）**、非所属の infra/external だけ従来 tier 帯に残る。既定ビュー（Group by: なし）の tier 体系（ADR-1724）は不変 — override は view-mode 局所。
