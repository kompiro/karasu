...
# ADR-1179: Monaco undo stack 統合 — `@monaco-editor/react` 経由で既に効いている
...
- **関連**:
  ...
  - 関連 ADR: [ADR-1076](./1076-gui-driven-style-editing.md)（GUI 編集器の親ルール、本件を当初 punt）、[ADR-1150](./1150-editor-external-refresh.md)（external write 受信側）

## 背景

ADR-1076 で「Monaco の undo スタック統合は MVP のスコープから
...
これは ADR-1076 が "ほしい" と書いていた挙動そのもの。
