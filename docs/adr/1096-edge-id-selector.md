...
topic: edges
depends_on: [ADR-1142]
related_to: [ADR-969, ADR-1076]
scope:
  packages: [core]
---

# ADR-1096: `.krs.style` の `edge#<id>` セレクタ — base ID + opt-in 著者 ID
...
- **関連**:
  ...
  - 関連 ADR: [ADR-969](./969-style-column-layout-hint.md)（node `#<id>` の specificity 階段に揃える）

## 背景

ADR-1076 の append-only round-trip で **個々の edge を一意に指す
...
specificity 100（ADR-969 の階段）に整合する。
