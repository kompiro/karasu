...
# ADR-1082: usecase resource operations に verb 装飾構文（1:N CRUD マッピング）を追加する
...
- **関連**:
  ...
  - ADR-1046: usecase 内 resource の `operations` プロパティ — 本 ADR はその syntax 拡張
  - ADR-1062: CRUD マトリクスビュー — 装飾済み verb を消費して `?` suffix を消す
  - 受け入れテスト: `docs/acceptance/1082-verb-crud-decoration.md`
  - 仕様: `docs/spec/syntax.md` § Verb-decoration syntax

## 背景

ADR-1046 の recognized set は `create` / `read` / `update` / `delete` の 4 種で、それ以外の verb（`list` / `search` / `enqueue` / `replace` …）は AST に保持されつつ `unknown-resource-operation` warning が出ていた。ADR-1062 のマトリクスビューはそれを `R?` のような `?` suffix で表示し、ユーザーに「CRUD verb で書き直して」と誘導していた。
