...
Thirteen Dependabot PRs were opened on 2026-04-20. Three are exact duplicates caused by the current `dependabot.yml` watching both `/` and each `/packages/*` directory (tracked separately in #771). Of the remaining ten, eight are patch/minor updates with the lockfile in sync, one is a patch update missing the lockfile update (vite), and one is a **major** runtime-library bump (marked 17 → 18).
...
| `vite` | 8.0.5 | 8.0.9 | dev (app) | #757 | Check ✗ (lockfile) |
| `marked` | 17.0.6 | 18.0.2 | runtime (app) | #758 | Check ✗ (lockfile) + **MAJOR** |
...
### vite 8.0.5 → 8.0.9 (lockfile missing)

Security-adjacent hardening in 8.0.9: the dev server now rejects HMR patch file requests from untrusted origins (no CVE assigned but worth adopting). Other fixes: `strictPort` wildcard handling, `emptyOutDir` in watch mode, CSS sourcemap key collisions on same-basename files, SSR class property key hoisting. No breaking changes. vite is used only in `packages/app`. #757 does not touch `pnpm-lock.yaml`, so CI fails; either ask Dependabot to rebase or regenerate the lockfile locally before merging.
...
- #757 and #758 require manual rebase to fix lockfile drift — small operational cost.
