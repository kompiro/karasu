...
# ADR-1381: マルチファイル import の意味論 — whole-file import / system 再オープン / DAG 再到達
...
- **関連**:
  ...
  - 関連 ADR: [ADR-281](281-wildcard-import-two-pass-resolution.md)（wildcard import / 2 パス解決）, [ADR-292](292-directory-import.md)（ディレクトリ import）, [ADR-412](412-named-import-toplevel-service.md)（top-level service named import）, [ADR-927](927-import-system-nested.md)（system 配下 named import の path 構文）
...
karasu の import 形式は ADR-281 以降に段階的に拡張されてきた。`import "p.krs"`（whole-file / wildcard）、`import { Foo } from "p.krs"`（named）、`import "dir/"`（directory）、`import { A.B.C } from "p.krs"`（system 配下 named — ADR-927）の 4 形式が存在する。しかし「1 つの `system` を複数ファイルに分割し、`index.krs` で `import "..."` を並べて束ねる」という自然なユースケース（Issue #1381）で次の挙動が起きていた:
...
- **B. whole-file import に新構文（`import * from "p.krs"`）を追加**: ADR-281 で既に検討・却下済み。冗長でメリットなし。本 ADR でも継承。
