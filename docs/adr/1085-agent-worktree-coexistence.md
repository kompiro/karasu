...
# ADR-1085: ユーザー作成 worktree は `.claude/worktrees/<branch>` 配下に置き、Claude Code Agent の自動 worktree と共存させる
...
- **関連**:
  ...
  - 関連 ADR: [ADR-1084](1084-skills-plugin-portability.md)（skills plugin portability — worktree path 統一の根拠）

## 背景

`kompiro/hane` plugin（ADR-1084）の前提として、ユーザーが手動で作る永続 worktree の配置先を統一する必要があった。候補は次の二つ:
