...
# ADR-1363: @karasu-tools/core を v0.x の公開パッケージにする（development 条件で build 非依存 typecheck を維持）
...
- **関連**:
  ...
  - 統治 ADR: [ADR-1314](1314-krs-spec-v1-freeze.md)（`.krs` は v1.0、**TS API は v0.x**）, [ADR-1315](1315-release-automation-changesets.md)（changesets 運用）
  - AT: `docs/acceptance/1363-publish-core-package.md`

## 背景

#1302 のハイブリッド版管理は「`.krs` / `.krs.style` 言語は v1.0、`packages/core` の
TS API は v0.x（無保証）」を定めた（[ADR-1314](1314-krs-spec-v1-freeze.md)）。
