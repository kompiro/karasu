...
# ADR-1632: deploy unit は共有 infra ノードを realize できる（store kind を新設）
...
- **関連**:
  ...
  - 関連 ADR: [ADR-316](316-database-as-first-class-node.md)（database/queue/storage を論理層の first-class node に昇格）
...
infra block keyword（`database` / `queue` / `storage`）は [ADR-316](316-database-as-first-class-node.md) で
