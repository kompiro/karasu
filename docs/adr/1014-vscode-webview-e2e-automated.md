...
# ADR-1014: VS Code WebView の DOM 系テストは ExTester ハーネスで自動化する

- **日付**: 2026-04-29
- **ステータス**: 決定済み
- **関連**:
  - ADR-863 — VS Code 拡張ホスト向け smoke test harness
  - ADR-926 — VS Code WebView の DOM 系テストはマニュアル運用とする（本 ADR が supersede）
...
ADR-926 は WebView を駆動する E2E ハーネスのコストが「対象 AT 4 件」
...
その過程で、ADR-926 が前提としていた「ROI が見合わない」「拡張に
...
- 拡張本体のバグ検出力が上がった：ExTester は **installed-mode の .vsix を
  ...
  （#1024）を検出できる。これは ADR-926 の手動 QA では拾えなかった。
...
3. **テスト追加方針**: 新規 WebView 系 AT は既存 suite に co-locate する
   ...
   踏襲する。
4. **拡張ホスト側スタブ禁止は維持**: ADR-926 のルール 3
...
- **Design Doc + Phase 1〜3 の実装で「不確実性」が解消された**。
  ADR-926 時点では「ExTester が xvfb 上で安定するか」「拡張の
...
| 案                                                          | 却下理由                                                                                                          |
| ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| **ADR-926 を維持して manual 運用に留める**           | ROI が逆転した。Phase 3 で投じたコストは harness 立ち上げ済みの今、追加 TC 移植にはほぼ転嫁されない。              |
...
- **`.claude/rules/vscode-webview-tests.md`** は ADR-926 の頃の
...
- **既存 ADR-926 の status を `superseded` に変更**し、
  `superseded_by: ADR-1014` を追加する。`pnpm adr:validate` が
  双方向参照を担保する。
