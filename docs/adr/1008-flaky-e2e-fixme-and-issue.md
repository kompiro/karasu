...
# ADR-1008: flaky な E2E テストは test.fixme でマークし追跡 Issue を立てる
...
- **関連**:
  - ADR-529: Playwright + AI visual review — E2E 実行基盤の前提
  - ADR-916: acceptance test automation markers — `at:auto` / `at:manual` の運用と整合
...
- **再 retry に頼る**: ADR-529 で確立した「flake は表面化させて潰す」方針と矛盾する
...
ADR-529 の趣旨（flake は visible にして潰す）と矛盾。Issue #1008 の動機そのものが「retries が flake を覆い隠している」だったので、本決定は #1008 の自然な延長。
