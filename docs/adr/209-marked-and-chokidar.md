...
# ADR-209: Markdown レンダリングに marked、ファイル監視に chokidar を採用
...
5. **Active maintenance** — marked publishes frequent releases and addressed two ReDoS vulnerabilities promptly (see ADR-199).
...
4. **ESM-first from v5** — chokidar 5 (currently in use) is ESM-only and requires Node.js ≥ 20.19, both of which align with the CLI package's existing `"type": "module"` declaration and the CI Node.js 22 baseline (see ADR-199).
