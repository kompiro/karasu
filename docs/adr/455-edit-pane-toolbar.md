...
# ADR-455: EditPaneToolbar — LeftPane アクションボタンの専用ツールバーへの集約

- **日付**: 2026-04-11
- **ステータス**: 決定済み
- **関連**: Issue #455, [ADR-465](465-edit-area-and-sidebar-toggle-relocation.md), [ADR-9009](9009-toolbar-icon-label.md)
...
設計時の案1 では `LeftPaneToolbar` として検討していたが、実装時に `LeftPane` 全体の責務整理（ADR-465 の `EditArea` 導入）に合わせて **`EditPaneToolbar`** にリネームした。`EditArea > EditPane > EditPaneToolbar` という階層が名前から読み取れるようになり、`Edit*` ファミリーとして命名体系が統一された。
...
- **ADR-465 との命名一貫性**: `EditArea` / `EditPane` / `EditPaneToolbar` の `Edit*` ファミリーで階層関係が名前から読み取れる
