...
- **関連**:
  ...
  - 関連 ADR: [ADR-968](./968-orthogonal-edge-routing-skip-layer.md)（B — エッジルーティング）
...
- **B との相補関係**: D で多くの貫通を解消し、B（[ADR-968](./968-orthogonal-edge-routing-skip-layer.md)）のオルソゴナルルーティングは A・D で救えなかった残りを処理する。
