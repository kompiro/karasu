...
# ADR-20260717-02: QA 手動チェックリスト生成のマーカー対応と 3-way triage

- **日付**: 2026-07-17
- **ステータス**: 決定済み
- **関連**: Issue #2045, ADR-20260412-05（三層 QA モデル）, ADR-20260324-01（旧・手動 QA 優先、superseded）

## 背景

ADR-20260412-05 は三層 QA モデル（Playwright 自動層 / AI 視覚レビュー層 / 手動 QA
...
2. **3-way triage**: 残った真の手動項目を `spec-target`（committed test で自動化可能）/
   ...
   する。triage は ADR-20260412-05 の三層に対応する advisory な振り分けであり、確定分類ではない。
...
- **三層モデルの道具化**: 3-way triage は ADR-20260412-05 が定めた三層の入口を機械化する。
...
- Issue #2045
- ADR-20260412-05（三層 QA モデル。本 ADR はその第 3 層の生成器を具体化する）
