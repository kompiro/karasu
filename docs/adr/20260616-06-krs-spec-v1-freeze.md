...
- **日付**: 2026-06-16
- **ステータス**: 決定済み（方針確定。v1.0 の公開確定は launch 時 — 下記参照）
- **関連**:
...
> v1.0 を「出荷」するわけではない。**現在 repo は private で、v1.0 の確定アナウンスは
> 出していない。** `.krs` / `.krs.style` を v1.0 と**公に確定するのは公開ローンチ時**
> （#1317）であり、本 ADR は方針と前提条件ゲートを定めるに留まる。

## 決定

**`.krs` / `.krs.style` を v1.0（stable）として凍結する方針を採択する。後方非互換な
spec 変更は v1.x ではなく v2.0 を要する。追加的変更は v1.x で許可する。TS API は
v0.x のまま。** v1.0 の公開確定は launch（#1317）で行う。
...
- [x] 既知の保留破壊的変更が解消済み — `team` property 削除（ADR-20260614-01）は
      ...
      は維持。
- [x] AT suite green（CI）。
- [ ] **公開ローンチ（#1317）で v1.0 を確定・アナウンスする** — repo を public にし、
      README / リリースで v1.0 を宣言する。本 ADR の発効点。
