...
# ADR-1554: エッジコンテキストメニューへの authored ラベル表示と data-edge-label の authored 専用化

- **日付**: 2026-06-13
- **ステータス**: 決定済み
- **関連**:
  - Issue #1554
  - ADR-463: 集約された暗黙エッジの詳細パネル — SVG 属性埋め込み方式
...
ADR-463 が `data-domain-edges` で確立した「SVG data 属性にモデル情報を
...
- **属性方式は既存系譜と一貫**。ADR-463 が集約暗黙エッジの内訳を
...
- **エッジの識別は引き続き `canonicalId`、ラベルは表示専用**。今回の変更は
  この境界（TPL-2167）を保つ。`data-edge-label` の有無や内容は識別に
