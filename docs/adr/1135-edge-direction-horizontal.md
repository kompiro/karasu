...
topic: edges
depends_on: [ADR-9019]
related_to: [ADR-395, ADR-969, ADR-1061]
refines: [ADR-9019]
scope:
  packages: [core]
---

# ADR-1135: edge `direction: left` / `direction: right` の layered layout 反映
...
- **関連**:
  ...
  - 親 ADR: [ADR-9019](./9019-edge-direction-style.md)
  - 関連 ADR: [ADR-395](./395-barycenter-layer-ordering.md)（layer 内 x の baseline）、
    [ADR-969](./969-style-column-layout-hint.md)（node `column` hint）、
    [ADR-1061](./1061-resource-rw-edges.md)（last-wins 慣習）

## 背景

ADR-9019 で `direction` enum の 5 値を定義したが、`left` / `right`
...
- **矛盾解決**: 同一 source への複数の矛盾するヒントは declaration 順で
  **last-wins**（ADR-1061 の cascade 規約と整合）
...
- **ADR-969（column hint）と直交**: column は absolute bucket
  指定、edge `direction` は二項関係を表す上位 override 層という関係に
  整理した。`bucketByColumn` の出力に重ねる形で実装するので、bucket
  境界をまたぐが整合性は崩れない
- **ADR-1061 の last-wins 慣習に整合**: GUI 編集器（#1129）の
