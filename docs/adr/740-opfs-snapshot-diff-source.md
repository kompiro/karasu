...
# ADR-740: OPFS 履歴スナップショットを diff 比較ソースにする

- **日付**: 2026-04-22
- **ステータス**: 決定済み
- **関連**:
  - Issue #740 (Closed), PR #786 (実装)
  - ADR-650 — グラフィカル diff ビューア（本 ADR の上位文脈）
  - ADR-739 — Diff ペースト入力の UI 配置とストレージ方式（`CompareSource` union の姉妹ケース）
...
Phase 1 の graphical diff viewer（ADR-650）は「ワークスペース内の別ファイル」だけを比較対象に取っていた。ProjectMode は OPFS を唯一のストアとして運用しており **git が存在しない** ため、「昨日の `index.krs` と今の `index.krs` を比べたい」というブラウザデモの素直な要求に応えられなかった。
...
| F 識別子 | F-2 `CompareSource` 判別 union | ADR-739（paste）と共用し、1 回の変更で両者を載せる |
