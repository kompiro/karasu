...
# ADR-1828: repo-backed + ref-pinned permalink（nest Phase 2 resolver）
...
- **関連**:
  ...
  - [ADR-1783](1783-karasu-nest-hosted-preview.md)（nest v1 = inline `?s=`、`/<owner>/<repo>` を Phase 2 として後続化。本 ADR がその Phase 2 を具体化）
  - [ADR-1827](1827-permalink-deep-element.md) / [`docs/spec/permalink.md`](../spec/permalink.md)（deep anchor grammar `#krs-<view>-<id>`）
...
#1828 が解く permalink 属性は **repo-backed（GitHub repo の `.krs` を解決して描画）** かつ **ref-pinned（特定 SHA に固定して immutable に描画）**。ADR-1783 が「Phase 2」として明示的に後続化していた `/<owner>/<repo>` resolver を、keystone 決定が funnel-only の後回し項目から retained-product backbone へ格上げした。本 ADR はその resolver の設計を確定する。
...
- **karasu-local の grep script で検証（ADR-1830 の再確認）**: 守る repo を間違える（実受益者は下流 repo）・再利用できない。検証は adr-tools に置く。
- **保存型ストア / 永続 paste（ADR-1783 で既に却下）**: SHA-keyed cache は ephemeral（Cache API の TTL 付き）に留め、永続レコードは taka（外部 D1）に閉じる。
