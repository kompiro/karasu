...
- **日付**: 2026-04-11
- **ステータス**: 決定済み
- **関連**: Issue #464, Issue #447, [ADR-20260409-02](20260409-02-cli-translate-command.md), [ADR-20260412-02](20260412-02-cli-mutation-subcommands.md)
...
- **stdin 経由**: 引数の数による挙動切替は直感的でない。`apply` と `append` / `insert`（ADR-20260412-02）の一貫したインターフェースになる
...
- `karasu remove <node-id> <krs-file>` → ADR-20260412-02
- `karasu append <krs-file>`（stdin） → ADR-20260412-02
- `karasu insert <parent-id> <krs-file>`（stdin） → ADR-20260412-02
