...
date: 2026-04-29
topic: cli
related_to: [ADR-650, ADR-739, ADR-740, ADR-811, ADR-1025]
assumptions:
...
# ADR-1020: `karasu diff` CLI と diff SVG の self-contained スタイル化
...
- **関連**:
  ...
  - 関連 ADR: [ADR-650](./650-graphical-diff-viewer.md)（graphical diff viewer）, [ADR-739](./739-diff-paste-input-ui.md), [ADR-740](./740-opfs-snapshot-diff-source.md), [ADR-811](./811-diff-open-file-as-entry.md)
  - 設計経緯: 旧 Design Doc は本 ADR で置き換え

## 背景

karasu には既に in-app graphical diff viewer（[ADR-650](./650-graphical-diff-viewer.md)）と、`data-diff-state="added | removed | changed | unchanged"` を SVG ノード/エッジに付与する rendering primitive がある。入力経路も paste（[ADR-739](./739-diff-paste-input-ui.md)）と OPFS snapshot（[ADR-740](./740-opfs-snapshot-diff-source.md)）が整備されている。
...
- ~~[#1025](https://github.com/kompiro/karasu/issues/1025) — bundled all-views diff（CLI 既定で 3 view を 1 SVG に束ねる）~~ — landed in [ADR-1025](./1025-bundled-all-views-diff.md)（PR [#1048](https://github.com/kompiro/karasu/pull/1048)）
