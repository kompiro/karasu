...
# ADR-1639: user は system-scoped とする（identity ではなく relationship）
...
- **関連**:
  ...
  - 関連 ADR: [ADR-1281](1281-user-role-keyword-clarification.md)（`role` は actor archetype、authz primitive ではない）, [ADR-832](832-no-runtime-authz-modeling.md)（runtime authz をモデル化しない）
...
- **`user` は relationship であって identity ではない**。`user` は特定 system との
  ...
  system*」と定義される（ADR-1281）。共有インフラは多数 system から参照される
