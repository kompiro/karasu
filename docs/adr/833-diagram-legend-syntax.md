...
# ADR-833: 図の凡例（legend）構文をモデル側に追加する
...
- **関連**:
  ...
  - ADR-9004 — CSS インスパイアのスタイリングシステム
  - ADR-8 — ビルトインスタイルの一元化と構造化リファレンス
  - ADR-813 — i18n ポリシー（凡例ラベルは exemption）
...
- **論理層に置く**: 凡例は「色 → **意味**」の宣言で、意味は著者がモデルに
  込めた意図。スタイルは表現のレイヤーで、両者を混ぜない（ADR-9004 の
...
- **i18n exemption**: `name` / `label` と同じ系統。著者の語彙そのものが意味の
  根拠なので app の locale で上書きしない（ADR-813 の例外条件、
