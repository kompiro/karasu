...
# ADR-1720: client は realizes / owns の対象になれる（valid-target に client kind を追加）
...
- **関連**:
  ...
  - 関連 ADR: [ADR-1632](1632-infra-physical-realize.md)（deploy unit は共有 infra ノードを realize できる）
...
`client` は [client capability modeling](837-client-capability-modeling.md) 系の決定で論理層の
...
[ADR-1632](1632-infra-physical-realize.md) 以降は infra）だけを列挙しており、`client` を含めていなかった。
結果として:
