...
# ADR-1886: 差分モードの Group by で除去ノードを元の team フレームに残し、集約エッジの diff state を再導出する（#1886）
...
- **関連**:
  ...
  - 前提 ADR: [ADR-1858](1858-system-view-group-by-team.md)（P2a: team 軸グループ化。本 ADR はその compare/diff モードでの噛み合わせ修正）
...
P2a（ADR-1858）の Group by: team を **compare/diff モード**（`compileSystemDiff`）で有効化した #1873 のレビューで、grouping と diff の噛み合わせに 2 つの誤表示が残った（#1886）。原因はどちらも、`compileSystemDiff` が grouping 軸を **after 側だけの `ownerIndex`** で render に渡していること（`diffed.slice` は before ∪ after の和集合なので、before にしか無いノード/エッジは after の ownerIndex で所属を解決できない）:
