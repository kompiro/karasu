...
# ADR-278: ブラウザ履歴ナビゲーション — URL hash による drill-down 同期

- **日付**: 2026-04-04
- **ステータス**: 決定済み
- **関連**: Issue #278, [ADR-110](110-permanent-link.md)
...
深い階層の回復（例: `#krs-system-EC` → `viewPath: ["Payment", "EC"]`）には `KrsFile.nodePathIndex`（ADR-110 Phase 1）を使う。

## 理由

- **リロード・URL 共有に対応**: `permanent-link.md`（ADR-110 Phase 2）の URL hash 方針と一致し、ここで実装しておけば将来的な統合作業が不要
