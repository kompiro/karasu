...
ところが #1564（[ADR-1564](1564-remove-team-property.md)）で per-node `team` プロパティを廃止したため、オーナーは `organization` ブロック経由でしか表現されなくなった。プロンプトに含まれるファイル本文は**現在開いているファイルのみ**であり、マルチファイル構成で `organization` ブロックが import 先のファイルに宣言されていると、その本文はプロンプトに現れない。結果として「X のオーナーは？」「X の連絡先は？」がマルチファイルで解決できなかった。
