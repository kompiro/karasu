...
date: 2026-07-17
topic: renderer
related_to: [ADR-20260711-03, ADR-20260713-01, ADR-20260716-03, ADR-20260630-02, ADR-20260611-02, ADR-20260715-03, ADR-20260716-01, ADR-20260714-04]
assumptions:
...
# ADR-20260717-01: boundary grouping の drill-down 拡張 — 描画レベルとの交差による正規化
...
- **関連**:
  ...
  - ADR: [ADR-20260711-03](20260711-03-system-view-group-by-team.md)（P2a team 軸 — 決定 7 の root-only を本 ADR が一般化）、[ADR-20260713-01](20260713-01-notation-promotion-gate.md)（notation promotion gate）、[ADR-20260716-03](20260716-03-group-by-team-multi-system-root-per-system-frames.md)（同一ラベル・disjoint フレームの先例）、[ADR-20260630-02](20260630-02-layer-toggles.md)（interactive collapse コントロールの gate — entity view の frames-only 挙動の根拠）、[ADR-20260611-02](20260611-02-legend-drill-down-scope.md)（per-drill-depth exact-match の先例）、[ADR-20260715-03](20260715-03-system-view-p2c-grouped-edge-routing-and-marks.md)（P2c routing/marks）、[ADR-20260716-01](20260716-01-group-by-diff-removed-node-placement-and-aggregated-edge-state.md)（diff-mode）
...
2. **同一 boundary の複数フレーム**: member が複数レベルに散る boundary は、レベルごとに同名ラベルの disjoint なフレームを持つ（[ADR-20260716-03](20260716-03-group-by-team-multi-system-root-per-system-frames.md) の multi-system per-(system, team) フレームと同型。1 枚の枠でレベルを跨いで囲まない）。
...
- **既存原則と整合**: [ADR-20260611-02](20260611-02-legend-drill-down-scope.md) の「レベルごとに、そのレベルのものだけを描く」（legend の per-drill-depth exact-match）、[ADR-20260714-04](20260714-04-expand-container-in-place.md) の「入れ子は drill-down の領域」と一致する（system view を deep 化しない）。
...
- **per-level axis（`boundary payments service { … }` 等のレベル指定を新設）**: member は id 参照であり、その id がどのレベルで描画されるかはモデル構造が既に決めている。flat index × 描画レベルの交差で同じ結果が得られ、著者に冗長な指定を課すだけ。experimental に corpus evidence ゼロで構文表面積を足すのは [ADR-20260713-01](20260713-01-notation-promotion-gate.md) の規律に反する。
