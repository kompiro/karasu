...
# ADR-1410: Outline ビューはアクティブビューの AST に追従する
...
ADR-1408 で導入した Outline ビューは、常に system AST
...
- **presentational コンポーネントの単一化**: 再帰描画・選択ハイライト・
  アイコン枠といった共通要素を `OutlineView` 1 か所に保てる。ADR-1408
...
- **ビューごとに専用 Outline コンポーネント**（`SystemOutline` /
  ...
  ADR-1408 の単一 `OutlineView` 方針から外れる。
