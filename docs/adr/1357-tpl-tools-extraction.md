...
# ADR-1357: TPL ツールを `@kompiro/tpl-tools` として外出しし、karasu からは外部 package 経由で参照する
...
- **関連**:
  ...
  - 関連 ADR: [ADR-1192](1192-test-perspective-library.md)（TPL 運用の決定）, [ADR-1296](1296-reference-data-single-source.md)（reference-data の single-source 設計）, [ADR-1077](1077-adr-config-externalization.md)（`topics` 語彙の `adr.config.json` 外出し）

## 背景

`scripts/tpl/` には Test Perspective Library（TPL — ADR-1192）を扱うツール群が約 1,100 LOC 置かれていた:
...
- **責務分離**: TPL ツールは「Markdown+frontmatter ドキュメント群を扱う汎用ツール」であり karasu の product code ではない。独立 repo に切り出すことで両者の release cycle を切り離せる。`adr-tools` と同じ取り扱い方針（ADR-1077 系の延長）で一貫する
...
- **`adr.config.json` をハードコードしない理由**: ADR を採用しないプロジェクトでも tpl-tools をそのまま使えるようにするため。karasu スコープでの single-source（ADR-1296）は wiring 側が `--config` を渡すことで実現できる
...
- **`topics` を中立な設定ファイル名（例: `reference-data.json`）に移す**: ADR-1296 を破壊的変更で書き直す必要がある。今回のスコープ外。tpl-tools 切り出しはこの判断と独立に進められる。3 つ目以降のツールが現れたタイミングで再検討する
