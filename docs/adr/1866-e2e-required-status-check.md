...
# ADR-1866: app E2E（Playwright）を Required status check にし、paired stub で永久 pending を防ぐ
...
app の Playwright E2E は ADR-1729 で path filter 駆動になり、
...
（ADR-1729）が、マージゲートとしては効いていない。壊れたアサーションが
自分の E2E run の完了前に `main` へ入りうる。

ADR-1729 は当時「`Playwright` を required にすると path 非該当 PR
...
大きい。path filter 化（ADR-1729）で得た「必要な PR だけ走らせる」利得を
捨てることになる。

### 案 C: required にせず、運用でマージ前に E2E 完了を待つ

**却下**: 人間が毎回 status を確認して待つ運用依存になり、ゲートとして機能
しない。ADR-1729 が opt-in ラベル運用を廃した理由（人依存のブラインド
スポット）と同じ轍。
