...
# ADR-1815: system view のコンテナをその場で展開する（in-place expansion / true mixed-LOD）
...
- **関連**:
  ...
  - 再利用した機構: [ADR-1858](1858-system-view-group-by-team.md)（group-by team の境界フレーム + 二段レイアウト）, [ADR-1872](1872-category-collapse-retarget-edges.md)（collapse 時の越境エッジ re-target）, [ADR-1821](1821-layer-toggles.md)（on-SVG affordance + interactive-only）, [ADR-226](226-drill-down-adapter-hierarchy-node.md)（drill-down）
  - 制約 TPL: [TPL-1223](../test-perspectives/TPL-1223-scoped-glance-drill-down.md)（scoped glance を first-class に保つ）, [TPL-1738](../test-perspectives/TPL-1738-relayout-into-group-preserves-placement-and-edges.md)（再配置時の全要素一度配置 + 端点保持）
...
重要な観測: **group-by team（ADR-1858）は既に「子を内包する境界フレームを畳んだ兄弟の隣に描く」機構を持つ**。in-place expansion はこの逆操作（team を 1 stub に畳む代わりに、1 コンテナを実サブレイアウトに開く）とほぼ同型であり、`assignGroupedLayers` / `buildGroupFrames` / エッジ re-target をそのまま再利用できる。
...
4. **affordance は on-SVG・interactive-only**（ADR-1821 踏襲）。畳んだ service に ⊕、展開フレームに ⊖（`data-expand-node`）。展開フレームは accent 枠 + 淡い塗りで俯瞰から目立たせる。static 出力・multi-system root・group-by team では出さない。
5. **scoped-glance ガードはソフト**（TPL-1223）。Phase 1 は単一展開で構造的に保護。Phase 2 は「Collapse all」で全展開を畳んで俯瞰へ戻し、同時展開が閾値（4）以上でツールバーに警告ヒントを出す。**ハード上限は設けない**。
6. **深い入れ子展開は不採用**（展開 domain の子をさらに展開するのは drill-down の領域）。

## 理由

- **既存機構の再利用で de-risk**: in-place expansion は group-collapse の逆であり、境界フレーム描画・二段レイアウト・エッジ re-target は ADR-1858 / ADR-1872 の資産をそのまま使える。Phase 1（単一展開）を案1 の最小ケースとして先に作り、難所（サイズ不連続をまたぐ frame 端点ルーティング）を de-risk してから複数展開へ広げた。
...
- **scoped glance の保護**: karasu の中核認知モデルは scoped glance（一度に見せる量を絞る、TPL-1223）。複数同時展開はこの原則と緊張するため、無制限展開を既定にせず「戻る導線 + ソフト警告」で牽制した。ハード上限にしなかったのは、比較・探索の自由をユーザーに残すため。
