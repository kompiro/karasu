...
# ADR-1742: VS Code E2E（extension host / WebView）もラベル駆動をやめ path filter で起動する
...
ADR-1729 は app の Playwright E2E をラベル駆動から path filter へ移行し、
...
**採用理由**: ADR-1729 / ADR-579 と同じ構造で一貫し、
...
**却下**: ブラインドスポットが残る。ADR-1729 で app E2E について退けた
理由がそのまま当てはまる。
...
- `paths` のホワイトリスト漏れリスクは追記で対応できる可逆な変更（ADR-579
  と同じ評価）。

## 将来の読者への注意

- ラベルでの手動トリガを再導入したくなったら、ADR-579 / ADR-1729 の
