...
# ADR-1104: translate adapter で usecase → resource バインディング + 任意 CRUD 装飾を emit する

- **日付**: 2026-05-06
- **ステータス**: 決定済み
- **実装**: PR #1143（Issue #1104）
- **関連**:
  - ADR-1082: verb 装飾構文 — 本 ADR が emit する装飾の入力先
  - ADR-1062: CRUD マトリクスビュー — 装飾を消費する側
  - ADR-1046: usecase 内 resource の `operations` プロパティ — 本 ADR が emit する構造
  - ADR-643: openapi resource grouping — 本 ADR が拡張する emit 元
  - ADR-644: db aggregate grouping — 本 ADR が拡張する emit 元

## 背景

ADR-1082 で `<verb>:<crud>[,<crud>...]` の装飾構文が parser に入り、ADR-1062 のマトリクスビューが装飾を読んで `?` suffix を消すようになった。一方で `karasu translate openapi` / `karasu translate db` は **そもそも `usecase` → `resource` バインディングを emit していない** 状態だった：
...
- **opt-in 維持**: ADR-1082 が「装飾はオプトイン」と明記している以上、バインディング emit も opt-in が一貫。既存ユーザーの translate 出力が無断で変わるのを避け、`apply` で既存ファイルにマージするユースケースで diff が爆発するのを防ぐ。
...
既存 `.krs` を再生成すると無関係な `resource` 追加で diff が増える。ADR-1082 が「装飾はオプトイン」と明記している以上、バインディングも opt-in が一貫。
