...
# ADR-458: Architecture レイアウトへの Barycenter + Sub-row wrap は適用せず共通ユーティリティのみ抽出

- **日付**: 2026-04-11
- **ステータス**: 決定済み
- **関連**: Issue #458, [ADR-392](392-deploy-layout-hierarchical-dag.md), [ADR-395](395-barycenter-layer-ordering.md)
...
`layers` パラメータを削除し y 座標のみで方向判定する案。`layout.ts` 本体への sub-row wrap 適用はしないため #458 のスコープでは不要と判断した。ただし `layoutMultipleSystems` への sub-row wrap 適用（Issue #467、PR #600）に際して実施済み。

### `layoutMultipleSystems` への同時適用（D-1）

スコープ拡大案として #458 では対象外とした。Issue #467（PR #600）で完了済み。
...
Issue #467（PR #600）にて `layoutMultipleSystems` への barycenter + sub-row wrap 適用を完了した。
合わせて B-1（`computeEdgePoints` の y 座標比較化）も実施し、`layers` パラメータを削除した。
