...
# ADR-1728: system-view の external サービスをサイド列に配置してエッジ交差を減らす
...
[ADR-1724] が dep ティアを infra 行と external 行に分割して横幅を半減したが、エッジの**交差**は減らなかった。`hato` の system view は 17 エッジ / 33 交差で、線が放射状に重なって追えない。実座標で交差を分解すると **28/33 が cross-hub 交差**（HatoApi × HatoMcp × WebApp など別ハブのファンアウト同士）だった。
...
- **補助の color-by-source**（[ADR-1755] の `edge[from=X]`/`edge[to=X]`）は配置で消せない残り交差の見分けに使える。

## 却下した案

- **直交トランク/バス・ルーティング**（[ADR-968] 拡張）: PoC で 33〜53 交差。cross-hub を対象にせず、ハブが infra 直上のため external は infra 帯を必ず縦断、素朴トランクは悪化。
