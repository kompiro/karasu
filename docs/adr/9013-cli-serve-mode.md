...
# ADR-9013: CLI `karasu serve` モード — ローカル `.krs` のリアルタイムプレビュー

- **日付**: 2026-03-28
- **ステータス**: 決定済み
- **関連**: [ADR-9006](9006-project-and-filesystem.md)
...
エディタ統合が最も自然で git 操作も VS Code が担うが、拡張機能のメンテナンスが必要で別パッケージ・別リリースフローになる。CLI 版とのコードベース分離管理も必要。serve モードが安定したあとに ADR-9014（LSP-first VSCode 拡張）として並行展開する方針にした。
