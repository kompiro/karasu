...
# ADR-1658: deploy view に service→infra 依存エッジを描く（導出は共有ヘルパーで単一情報源化）
...
- **関連**:
  ...
  - 関連 ADR: [ADR-1632](1632-infra-physical-realize.md)（deploy unit が infra を realize / `store` kind）
...
[ADR-1632](1632-infra-physical-realize.md)（#1632）で `store` が infra を realize できるようになったが、
