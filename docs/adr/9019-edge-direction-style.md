...
date: 2026-05-06
topic: edges
depends_on: [ADR-1142, ADR-1096]
related_to: [ADR-1135, ADR-1076]
scope:
  packages: [core, app]
---

# ADR-9019: `.krs.style` の edge `direction` プロパティ — 矢印の流れる向きを 5 値 enum で指定
...
- **関連**:
  ...
  - 親 ADR: [ADR-1142](./1142-gui-style-inplace-update.md)（旧: [ADR-1076](./1076-gui-driven-style-editing.md)、本決定で superseded）、
    [ADR-1096](./1096-edge-id-selector.md)
  - 派生 ADR: [ADR-1135](./1135-edge-direction-horizontal.md)（`left` / `right` を refine）
...
ADR-1076 の GUI 駆動編集が固まったことで、判断軸が変わった:
...
- **GUI の append flow と整合**: ADR-1076 の cascade-tail-wins と
...
- ADR-1135: `left` / `right` の refinement（横方向の honor）

## スコープ外

- `left` / `right` の詳細仕様 → ADR-1135
