# ADR-0034: ブラウザ履歴でのハイライト復元 — hash コロン拡張

- **日付**: 2026-04-11
- **ステータス**: 決定済み
- **関連**: Issue #425, [ADR-0031](0031-atomic-highlight-on-cross-navigation.md), [browser-history-navigation.md](../design/browser-history-navigation.md)

## 背景

ADR-0031 (#422) でクロスナビゲーション時のハイライトが適用されるようになったが、`highlightedNodeId` は URL hash に反映されないためブラウザバック/フォワードでハイライトが失われていた。例: D ボタン → Deploy 図でハイライト → 別ビューに移動 → 戻る → Deploy 図は復元されるがハイライトなし、となっていた。
...
- **実装コスト最小**: `buildHash` / `parseHash` の変更が最小限で済み、`SET_ACTIVE_VIEW` は既に `highlightNodeId` フィールドを持つ（ADR-0031）ため呼び出し側の変更も少ない
...
既存の hash 形式との後方互換がなく、`browser-history-navigation.md` の設計を大きく崩す。
