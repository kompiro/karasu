...
topic: cli
authors: [kompiro]
related_to: [ADR-1314, ADR-1895, ADR-355]
scope:
  packages: [cli, core]
assumptions:
...
# ADR-1935: --from wrangler translate adapter と「adapter を採る基準」

## 背景

reverse-architecture ハーネス（[ADR-1895](1895-reverse-architecture-harness.md)）を
...
（[ADR-1314](1314-krs-spec-v1-freeze.md) の v1 freeze と非衝突）。
...
- **物理層の hallucination 回避**が中核。具体技術を `label` ではなく物理 `store { type ... realizes }`
  ...
  （[ADR-1895](1895-reverse-architecture-harness.md) の「物理層の hallucination 回避」）。
...
- ADR: [ADR-1314](1314-krs-spec-v1-freeze.md)（v1 freeze）、[ADR-1895](1895-reverse-architecture-harness.md)（reverse-architecture harness）、[ADR-355](355-cli-translate-command.md)（translate CLI）
