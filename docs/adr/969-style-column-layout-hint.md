...
date: 2026-04-29
topic: styling
related_to: [ADR-968, ADR-974, ADR-458]
assumptions:
...
# ADR-969: `.krs.style` 側の `column` で layer 内 x 配置を上書きする escape hatch
...
- **関連**:
  ...
  - 兄弟 ADR:
    - [ADR-968](./968-orthogonal-edge-routing-skip-layer.md)（B — skip-layer エッジルーティング）
    - [ADR-974](./974-infra-row-by-deepest-consumer.md)（D — infra/external row 引き上げ）
  - 関連 ADR: [ADR-458](./458-arch-layout-barycenter-wrap-scope-reduction.md)（layer 内 x 順序ヒューリスティクス）
  - 設計経緯: 旧 Design Doc は本 ADR で置き換え

## 背景

A（actor row 再配置 [#967](https://github.com/kompiro/karasu/issues/967)）・
B Phase 2（skip-layer 直交ルーティング [ADR-968](./968-orthogonal-edge-routing-skip-layer.md)）・
B Phase 3（port distribution + channel lane allocation [#996](https://github.com/kompiro/karasu/issues/996)）・
D（infra/external 引き上げ [ADR-974](./974-infra-row-by-deepest-consumer.md)）が
