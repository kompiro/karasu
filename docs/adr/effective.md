...
Auto-generated by `pnpm adr:regenerate`. 155 of 163 ADRs are currently `status: accepted` with no `superseded_by`.
...
- [ADR-20260509-04](20260509-04-edge-label-position-offset.md) — edge `label-position` / `label-offset` プロパティ — fractional 位置と 1 軸 perpendicular ずらし
