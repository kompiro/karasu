...
Auto-generated by `pnpm adr:regenerate`. 163 of 171 ADRs are currently `status: accepted` with no `superseded_by`.
...
- [ADR-20260512-02](20260512-02-feature-samples-builtin-project.md) — `examples/feature-samples/` を built-in ProjectMode example（1 プロジェクト）として同梱
