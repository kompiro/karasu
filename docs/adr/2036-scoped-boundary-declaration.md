...
- **関連**:
  ...
  - TPL: [TPL-1352](../test-perspectives/TPL-1352-composite-key-must-cover-all-distinguishing-dimensions.md)（scoped index / group id は (scope, id) でキー — 本件の中核観点）、[TPL-1983](../test-perspectives/TPL-1983-view-state-gate-parity-across-surfaces.md)、[TPL-1503](../test-perspectives/TPL-1503-accepted-vocabulary-must-have-effect.md)、[TPL-1101](../test-perspectives/TPL-1101-round-trip-guarantee.md)（ネスト構文の fmt round-trip）、[TPL-2032](../test-perspectives/TPL-2032-reference-existence-validated-on-merged-space.md)（scoped 診断の merged-space 再導出 — slice A の再発事例）
...
- **identity を（scope, id）にしても壊れるものが無いことを機構ごとに検証済み** — permalink（boundary はアンカー名前空間に不参加、[ADR-1827](1827-permalink-deep-element.md)）、style（boundary セレクタ不在）、collapse（`groupStubId` の scope 引数 + scope-qualified group id）。キーのエンコードは `boundaryScopeKey` / `scopedBoundaryGroupId` に一元化し injective（separator join は quoted id で衝突しうる — [TPL-1352](../test-perspectives/TPL-1352-composite-key-must-cover-all-distinguishing-dimensions.md)）。
