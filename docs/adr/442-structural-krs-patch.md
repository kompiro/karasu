...
# ADR-442: 構造的 `.krs` パッチ — ノード ID ベースの `append` / `replace` / `remove`

- **日付**: 2026-04-10
- **ステータス**: 決定済み
- **関連**: Issue #442, [ADR-419](419-chat-ui-phase2-byok-ai-integration.md), [ADR-362](362-chat-ui-panel.md), [ADR-469](469-cli-mutation-subcommands.md)

## 背景

ADR-419 の Phase 2 実装では、AI の `apply_krs_patch` ツールがスニペットをファイル末尾に追記する（`fileContent + "\n" + patch`）だけだった。これでは以下のユースケースに対応できない：
...
親ブロックの閉じ `}` 直前に挿入する専用オペレーションは実装コストが高いため、「親ノード全体を `replace`」で自然に表現する（AI が親ブロック全体を書き直す）。ADR-469 で CLI の `karasu insert` が `insert-child` オペレーションを追加したが、本 ADR の時点では `replace` で代替する方針だった。
...
（後に ADR-464 で `applyKrsPatch` は `@karasu-tools/core` に移動された）
...
`insert-child` の実装コスト（親ブロックの閉じ `}` 直前への挿入、インデント正規化）が本 ADR の時点では見合わないため、案D の `replace` での代替を採用した。後に ADR-469 で CLI 側に `insert-child` が追加された。
