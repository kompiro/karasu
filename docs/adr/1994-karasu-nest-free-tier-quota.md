...
**Workflow インスタンス id は試行ごとに変える。** id は `(installation, commit 12 桁, 課金後の使用回数)` から作る。インスタンス id はプラットフォームの保持期間のあいだ一意で、run の生存期間だけではない。commit だけで鍵にすると、**一度失敗した生成はその commit では二度と起動できなくなる** — 新しい commit が積まれるまで、すべての再試行がプラットフォームに拒否される。初版がそうなっており、しかも本 ADR の「再 POST すれば quota を 1 消費してやり直せる」という会計はその経路が存在することを前提にしていた。使用回数を混ぜると、同じ課金に競合した 2 人は同じ id（＝二重起動は起きない）、後からの再試行は別 id（＝やり直せる）になる。

**charge は dispatch の前、スロットは dispatch の後。** 非対称なのは、外し方の被害が非対称だからである。課金が早すぎれば返金で済む（check と create の間は 2 人目がすり抜ける窓なので、遅らせるほうが高くつく）。スロットが早すぎると、失敗した create が**誰も持ち主のいないスロット**を残す。それを解放するはずの Workflow は起動していないので、同時実行 1 のサービス全体が 90 分止まる。初版はスロットを先に取っており、レビューで指摘されて入れ替えた。**代償として check と takeSlot の窓が広がった**（KV 読み + KV 書きだったものが、GitHub API 呼び出し + KV 読み + create になった）。異なる repo に対する同時 POST 2 本が両方 dispatch されうるということで、これは上記のとおり同時実行が soft bound である以上、想定内の緩みである。請求を有限にしているのは月次 quota のほう。

**create が失敗したら、走っているかどうかを店に訊く。** 初版はすべての失敗を「重複だから既に走っている」と解釈して 202 を返していた。走っていない run を「走っている」と答えれば呼び出し側は永久にポーリングする。かといって一律 503 にすると、同じ課金に競合して負けた側（勝者の run は実際に走っている）に嘘をつく。so 失敗したら run 記録を読み直し、`running` になっていれば 202、なっていなければ 503 を返す。
...
**アンインストールで台帳も消える。** 月次カウンタ（`quota/`）と in-flight スロット（`busy/`）の**両方**。スロットは 90 分で自然に消えるが、「いずれ消える」は決定 6 が約束していることではないし、鍵には owner と repo の名前が入っている。ADR-1990 決定 6 に例外は無い（[TPL-2226](../test-perspectives/TPL-2226-every-key-prefix-must-be-purgeable.md)）。再インストールで月がリセットされる副作用は受け入れる。**理由は費用の非対称性ではない** — アンインストールと再インストールは 30 秒のクリック作業で、上限価格なら $10.80 相当を買える。受け入れるのは決定 6 に例外を作らないためで、残る濫用は「installation の付け外しが目に見える形で繰り返される」という、気づける振る舞いに縛られている。気づいたら installation 単位で断ればよい。
...
**日次・時間あたりの rate limit。** 月次で支出は有界になっている: 最悪でも 3 × installation 数 × $3.60/月（再インストールによるリセットは上記のとおり別枠で、目に見える）。この bound が成り立つのは step のリトライを 0 にしてあるからで（上記）、既定のリトライ方針のままだとリトライ上限倍に膨らんでいた。同時実行 1 は速度をならすだけで、上限を作っているのは月次のほうである。3 番目の軸は、断られ方を増やして説明を難しくするだけで、防ぐ支出が無い。
...
## 追記（2026-08-05）— 実測が取れた。水準は据え置く

`ddd-by-examples/library` の fork に対して実サービス経路で 1 回完走し、`GET /admin/metrics` が初の実測を返した。詳細は `docs/design/2226-nest-cost-model.md`。

| | 本 ADR の根拠（投影） | 実測 |
| --- | --- | --- |
| 1 回あたり | 想定 $2.15 / 上限 $3.60 | **$0.83** |
| 所要時間 | 12〜19 分 | **3〜4 分** |

**決定は変えない。** 理由は 2 つある。

第一に、**実測は本 ADR の根拠を否定していない**。構造的上限 $3.60 は `MAX_TOKENS` と `DEFAULT_MAX_BYTES_READ` から導いた値で、実測が下回ったのは典型例が上限に届かなかったからにすぎない。上限は上限のまま有効で、10 installation × 月 3 回が最悪 $108 という見積りも変わらない。

第二に、**実測は n=1 である**。1 repo・1 回・164 ファイル。入力側がコストを決めていると分かった以上、repo 規模の分布こそが最大の未知数で、それは他人の repo に向けられない現状では埋まらない（ADR-1990 決定 6）。

水準を上げる余地は生まれた。上げるときは新しい ADR で supersede する — **本節はその材料であって、決定の差し替えではない**。この ADR の数値を引用するときは、投影であることと、実測が n=1 であることの両方を併記すること。
