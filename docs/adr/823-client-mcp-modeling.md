...
# ADR-823: クライアント / MCP を system 図でどう表現するか — `client` kind の導入

- **日付**: 2026-04-25 (Design Doc 作成) / 2026-04-26 (Q1–Q14 全決定) / 2026-04-28 (ADR 昇格)
- **ステータス**: 決定済み — Phase 1〜7 実装完了、Phase 8 (本 ADR) で文書整備完了
- **関連**: Issue #823, Issue #832 (認可は別 Issue — 結論は ADR-832), Issue #834 (security 関連トピックの親 — credential / cookie 等はそちらで扱う), Issue #837 (capability 軸 — camera / geolocation 等の能力許諾), ADR-843 (feature toggle policy — 構文追加では非適用), ADR-832 (usecase レベル authz は語彙化しない), `docs/spec/syntax.md`, `docs/spec/tags-annotations.md`, `docs/concepts.md`
...
- **ユーザー属性に応じた usecase の認可** (ロール / ライセンス / プラン / 機能フラグ) — Issue #832。
  ...
  → その後 **ADR-832** で「usecase レベルの authz は karasu の語彙に取り込まない」と決定した。ここで挙げた予約候補は「将来 authz 機能を作るとき用」ではなく「衝突回避のための予約」という位置づけになり、`usecase.requires` を実装する計画はない。
...
MVP を一括実装すると PR が大きくなりレビューしづらいため、**8 フェーズに分割**して順次マージする。各フェーズは独立してマージ可能。Phase 1 が完了すれば Phase 2〜6 は並列で進められる。

### 依存関係

```
Phase 1 (client kind 最小)
    ├──► Phase 2 (icon)
    ├──► Phase 3 (delivers)
    ├──► Phase 4 (handles + 再エクスポート)
    ├──► Phase 5 (resource)
    └──► Phase 6 (強制レイアウト)
              │
              ▼
...
- **Phase 1** 完了が必須前提
- **Phase 2〜6** は Phase 1 後なら互いに並列可能（DB のような共有状態がなく、AST / renderer の独立した部分に閉じる）
- **Phase 7**（examples 拡充）は Phase 4 + 5 後に着手（handles / resource を含む完全な例を書くため）
- **Phase 8**（docs 仕上げ + ADR 昇格）は MVP 全機能が揃ってから

### 各 Phase の詳細

| Phase | 内容 | 規模目安 | 備考 |
|---|---|---|---|
| 1 | `client` kind 最小実装 (parser/AST/validator/renderer) + `examples/getting-started/` の `client` 追加 + `docs/spec/syntax.md` の最小記述 | 500–800 行 | 7 サブタイプタグ (`mobile`/`web`/`desktop`/`cli`/`device`/`extension`/`embed`) を予約。renderer は既定 card のみ（icon は Phase 2） |
| 2 | builtin icon (`packages/core/icons/client.svg` + `icons.json` エントリ + `icon-theme.ts` ルール) | 100–200 行 + SVG | `service` と視覚的に明確に区別。サブタイプ別 icon は MVP 外 |
| 3 | `service.delivers <ClientId>` プロパティ | 200–400 行 | `[external]` は既存タグなので追加実装不要 |
| 4 | `handles` cross-reference + `service.handles` 再エクスポート + 接続トポロジ検査 warning | 600–1000 行 | 1 ホップずつ展開する expose 検査が中心 |
| 5 | `client { resource <storageKind> "<name>" }` (6 kinds) | 400–600 行 | kind ごとに per-kind 検証を入れる。renderer は client 内テキスト一覧で十分 |
| 6 | 強制レイアウト (`user → client → service`) | 500–1000 行 | client 不在時は二層 fallback。golden SVG テスト多め |
| 7 | examples 拡充: `examples/client-mcp/` 新設 + `examples/ec-platform/02.5-clients.krs` 追加 | 200–400 行 | `getting-started` は Phase 1 で更新済 |
| 8 | docs 仕上げ + ADR 昇格: `docs/spec/syntax.md` 完全版 / `docs/concepts.md` 三軸説明への `client` 追加 / `docs/design/client-mcp-modeling.md` の ADR 化 | 200–400 行 | 全機能が揃ってから書く |

### 各 Phase の Issue 化

- 本 Doc が PR でマージされた後、各 Phase を独立した Issue として作成する
- すべての Phase Issue は `Refs #823` で本親 Issue に紐付ける
- `status: ready` / `status: blocked` ラベルで Phase 1 との依存を表現
- 並列着手可能な Phase は `status: ready` を付ける

### Phase 1 着手時の注意

Phase 1 は本 MVP の地ならし。以下を**意図的にシンプルに保つ**:
- renderer は既定 card のみ（icon は Phase 2 でリッチにする）
- レイアウトは現行のまま（強制 3 層は Phase 6）
- 例は最小（client が 1 つあるだけ）

これにより Phase 1 PR が肥大化せず、後続 Phase との切り離しが明確になる。

### Feature toggle policy との関係（ADR-843）

ADR-843 では feature toggle を「UI のエントリポイントを隠すための短寿命のコンパイル時定数」と定義し、**パーサー拡張・レンダラー・ストア等は原則として常時ビルドし常時テストする**と明記している。
これに従い、本 MVP の構文追加 (`client` kind / `delivers` / `handles` / `resource`) では **feature toggle を使わない**。

**理由**:
- 構文そのものがユーザーから見えるエントリポイントであり、UI ボタンのように 1 箇所で隠すことができない（parser は通るが renderer は無視、のような不整合状態を作りやすい）
- 8 Phase 全体を 1 つのトグルで囲うと数週間〜数ヶ月の長寿命化を招き、ADR の「数 PR / 数週間」規範に反する
- ADR が却下した「`/labs` 区画」と同じ性格の抜け道になる

**代わりに採る方針**: 各 Phase を **独立した小さな卒業 (graduation)** として扱う。
Phase 1 終了時点で `client X [tag] {}` が書けて render される正式な機能となり、Phase 2〜6 は Phase 1 の上に**それぞれ独立した機能拡張**として段階的にマージする。
Phase 1 後・Phase 3 前のユーザーが `client X { delivers Y }` を書くと `delivers` 部分で parse error になるが、これは「未対応構文を書いたら error」という通常の言語進化と同じ扱いで問題ない。

**例外: UI 機能を `client` の上に追加する場合**:
構文追加そのものではなく、`client` を活用する **UI 側のエントリポイント**（例: client 専用 detail panel、ノード storage 一覧表示など）を MVP 後に追加するときは、ADR-843 に従って通常の feature toggle で囲んで開発・卒業する。
これは本 MVP のスコープ外。

**記録**: 本 Phase 計画における各 Phase はそれぞれが「正式リリース」であり、spec docs (`docs/spec/syntax.md`) と examples は **Phase ごとに同 PR で更新する**。半完成状態のまま docs に残さない。
