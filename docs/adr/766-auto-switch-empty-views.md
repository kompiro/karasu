...
# ADR-766: 空ビューを避けるための自動タブ切替（system > deploy > org）
...
- **関連**:
  ...
  - PR #844 — org-only 実装
  - ADR-9003 — 論理構造と物理構造の分離
  - ADR-29 — Deployment Diagram Design Decisions
  - ADR-309 — Org tree view
...
ADR-9003 は「論理 (`system`) と物理 (`deploy`) の分離」を述べており、
ADR-309 は組織図 (`organization`) を独立した第三の関心軸として
...
- ADR-9003 が定める「論理が中心、物理は補助」の関心軸ヒエラルキーに整合。
...
却下理由: ADR-9003 の「論理/物理の分離」を破る。現在の core は
