...
# ADR-1415: Outline ビューはタグ駆動アイコン variant を core 共有関数で解決する
...
ADR-1408 で導入した Outline ビューは各ノードの Icon Mode
...
- **解決ロジックの単一集約**: アイコン語彙の解決を core の 1 関数に集約し、
  ...
  ADR-1408 の「presentational な単一 `OutlineView`」方針を崩さない。
...
- **`OutlineView` 内にタグ分岐を直書き**: 変更は app に閉じるが、アイコン
  語彙が `icon-theme.ts` と `OutlineView.tsx` に二重化し、ADR-1408
