...
# ADR-1566: `duplicate-owner-assignment` を info（fact-vs-style register）に下げる
...
本決定は ADR-14 §6 の「同一 ID を複数チームが owns → warning」を **info に改める**（旧 ADR は歴史的記録として残す）。
...
- **warning（ADR-14 §6 準拠）**: lossy さは表現できるが `domain-dispersal`(info) との非対称が残り、「直すべき」を含意しすぎる。
