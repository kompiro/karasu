...
- **テーマ・スタイルとの相性**: 44px 幅は karasu の `--bg-void` ベースの暗色テーマに馴染み、
  既存のツールバーボタン規約（ADR-307）と独立した別レイヤーとして共存できる。

## サイドバーパネルを追加する手順

1. アクティビティバーに新しいボタン（icon + text label, ADR-307 のラベル規約に従う）を追加する。
...
- ADR-307（toolbar button のラベル規約）— アクティビティバーボタンも icon + text label を保持する
