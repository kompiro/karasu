...
# ADR-2092: TPL の reference-data 設定を `tpl.config.json` に分離し、TPL は `date-sequence` を維持する

- **日付**: 2026-07-20
- **ステータス**: 決定済み
- **関連**:
  - 引き金 Issue: [#2083](https://github.com/kompiro/karasu/issues/2083)（ADR 採番の `issue-number` 移行 — 本 ADR はその Phase 1）
  - 前提 ADR: [ADR-1357](1357-tpl-tools-extraction.md)（tpl-tools 切り出し。`--config adr.config.json` を渡す wiring を決めた ADR — 本 ADR はその config 共有部分のみを改める）, [ADR-1296](1296-reference-data-single-source.md)（reference-data single-source）, [ADR-1077](1077-adr-config-externalization.md)（語彙の外部化）
...
ところが `@kompiro/tpl-tools` は ADR-1357 の決定に従い
...
- **`topics` 共有は維持する価値がある**: ADR-1296 の single-source 原則の
...
- **上流を変えるより呼び出し側で分ける方が軽い**: `tpl-tools` に「ADR とは別の
  ...
  事情が漏れる（ADR-1357 が避けたかったこと）。`--config <path>` を
...
- **ADR-1357 を `superseded` にする**: 本 ADR が改めるのは「karasu が
