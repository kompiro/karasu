...
# ADR-1168: `.krs.style` AST に位置情報と sheetId を持たせ、parser の error recovery を明示する
...
- **位置情報の欠如**: AST に `loc` が無いため、LSP はエラー位置を
  ピンポイントで指せず、GUI の in-place update（[ADR-1142](./1142-gui-style-inplace-update.md)）も
