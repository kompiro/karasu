...
# ADR-1144: GUI コンテキストメニューの append 先解決 — `.krs.style` 直接編集時の優先と target 表示
...
- **関連**:
  ...
  - 親 ADR: [ADR-1076](./1076-gui-driven-style-editing.md)（GUI 編集器の親ルール）

## 背景

ADR-1076 の append-only round-trip では、`resolveStyleAppendTarget`
...
- **target を可視化することで append 操作の安全性が上がる**: 複数の
  ...
  クリック前に確認できる。ADR-1076 の append-only / cascade-tail
...
- 却下理由: ADR-1076 の方針（GUI が最後の `@import` を見て決める /
