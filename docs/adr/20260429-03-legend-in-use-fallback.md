...
assumptions:
  ...
  - "grep: packages/core/src/renderer/svg-builder.ts :: LEGEND_MUTED"
---
...
- 解決チェーン:
  ...
  2. ref のターゲットが実ノードに付いている（`legendRefHasUsage(target, usage) === true`）→ neutral fallback swatch（`#9CA3AF`、`LEGEND_MUTED` と同色）（**新挙動**）
  3. それ以外 → `null` → 凡例から drop（**従来通り。`legend-ref-unresolved` 警告が出る**）
- フォールバック色は legend 内の muted text に既に使っている `LEGEND_MUTED` を流用し、別定数を新設しない。
