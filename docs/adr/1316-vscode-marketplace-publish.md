...
# ADR-1316: VS Code 拡張を Entra ID + GitHub OIDC（managed identity）で Marketplace に publish する
...
[ADR-1315](1315-release-automation-changesets.md) は npm 公開を changesets で
...
3. **認証は Microsoft Entra ID + GitHub OIDC（PAT 不使用）。** `azure/login`（federated credential、
   ...
   （[ADR-1315](1315-release-automation-changesets.md) の `release.yml` における
...
- **changesets と独立**: npm registry と Marketplace は配布先・認証（`NPM_TOKEN` vs Entra）・成果物
  （tarball vs `.vsix`）が異なる。[ADR-1315](1315-release-automation-changesets.md) の決定を踏襲し別配線にする。
...
- **changesets フローへ統合**: [ADR-1315](1315-release-automation-changesets.md) の決定に反する。配布先・認証・成果物が異なり分岐が増える。
