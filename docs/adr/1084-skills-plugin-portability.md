...
# ADR-1084: portable な開発スキルは `kompiro/hane` plugin に切り出し、karasu からは plugin 経由で読み込む
...
- **関連**:
  ...
  - 先行事例: [ADR-1357](1357-tpl-tools-extraction.md)（`@kompiro/tpl-tools` 外出し）
...
- **Worktree path の統一**: `.claude/worktrees/<branch-name>` を全 skill で前提化する（前提検証は ADR-1085 で扱う）
...
- **karasu 固有要素の除去**: Cloudflare Pages preview URL (`*.karasu.pages.dev`)、`pnpm at:check-coverage`、`/workspaces/karasu` の絶対パス、ADR-579 や `.claude/rules/adr.md` への直接参照、`#916/#918/...` 等の karasu Issue 番号は plugin 本体から削除する

## 理由

- **責務分離**: portable skill は karasu の product code ではない汎用ワークフロー。`@kompiro/tpl-tools`（ADR-1357）と同じ立て付けで独立 release cycle にすると、両者の改修が干渉しなくなる
...
- **既存 ADR との整合**: ラベル運用は ADR-579（preview workflow 無ラベルゲート）と整合し、ADR 昇格 auto-merge は `.claude/rules/adr.md` の運用に乗る。これらはどちらも karasu 側に残るため、plugin 側を optional 化することで karasu の運用は変えずに plugin だけ portable にできる
