...
- **引用符で囲むのは join を曖昧にするセグメントだけにする。** `.`・`"`・`\` のいずれも含まず空でもない id は以前と同じ綴りで出るので、そうした id だけでできたモデルでは id が 1 つも変わらない
...
- `.`・`"`・`\` を含む id も空の id も持たないモデル: コンテナ id は変わらない
- ノード id が `.`・`"`・`\` を含むか空であるモデル: コンテナ id に引用符が付く（`"www.example.com"`）。D ボタン、詳細パネルの `hasDeployContainer`、draw.io のタグ／アノテーションは `nodeId` 経由で保たれる
...
- `nodePathKey` の docstring は、`ownerIndex` を dotted join でキーにする理由を「JSON にすると dotted id を持つ既存消費側が壊れる」としている。`nodePathRefId` は `.`・`"`・`\` を含まず空でもないセグメントだけの path に対して素の join と同じ文字列を出すので、その前提はもう成り立たない。ただし本 ADR はその判断を覆さず、#2819 で扱う
