...
# ADR-1370: リリースを workflow_dispatch 起動の Prepare → release PR → マージで publish にする

- **日付**: 2026-06-23
- **ステータス**: 決定済み
- **関連**:
  - 設計検討 PR: [#1701](https://github.com/kompiro/karasu/pull/1701)（旧 `docs/design/release-flow-actions-driven.md` — 本 ADR に集約して削除）
  - [ADR-1315](1315-release-automation-changesets.md) — changesets 採用・publish-only。本 ADR はその「リリースの流れ」（ローカル `changeset version` → release PR → マージ → publish）の **trigger / version-bump の起こし方**を更新する
  - [ADR-9020](9020-npm-trusted-publishing-oidc.md) — publish 認証を OIDC trusted publishing 化（同じ `release.yml` を扱う）
...
1. `release.yml` は OIDC trusted publishing 化済み（ADR-9020）。
...
3. **`release.yml`（publish）の trigger を `push: branches:[main], paths: ["packages/**/CHANGELOG.md"]` + `workflow_dispatch`** にする。`changeset version` は CHANGELOG.md を必ず書き換えるため、release PR のマージ時にだけ発火し、毎 push のフルビルド浪費を避ける。publish 本体は OIDC（ADR-9020）のまま。
