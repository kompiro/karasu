...
# ADR-14: Organization 図（organization / team / member）の追加

- **日付**: 2026-03-23
- **ステータス**: 決定済み
- **関連**: Issue #14, [docs/spec/syntax.md](../spec/syntax.md), [ADR-9009](9009-toolbar-icon-label.md)

## 背景

従来の karasu は ADR-9003 の論理/物理分離方針に基づき、**論理図**（`system` / `service` / `domain` / `usecase` / `resource`）と**物理図**（`deploy`）の 2 種類のビューを持ち、サービス・ドメインのオーナーシップは `service { team "文字列" }` で表現していた。しかしこの方式には：
...
`viewKind: "logical" | "physical" | "org"` で切り替え、ツールバーボタン（ADR-9009 に従いアイコン + ラベル）を追加。`useOrgView` フックを `useKarasu` と並列に呼び出す。
...
- エッジクリックによる System 図ドリルダウン（ADR-110 で Phase 1 として対応）
