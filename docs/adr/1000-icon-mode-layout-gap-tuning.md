...
# ADR-1000: Icon display mode 用の auto-layout gap 定数を別系統に分ける
...
- **関連**:
  ...
  - 関連 ADR:
    - [ADR-458](./458-arch-layout-barycenter-wrap-scope-reduction.md)（barycenter ordering + sub-row wrapping）
    - [ADR-968](./968-orthogonal-edge-routing-skip-layer.md)（skip-layer 直交ルーティング）
...
- **column hint との粒度差を明確化**: `.krs.style` 経由で公開するか
  という議論は別。column hint（[ADR-969](./969-style-column-layout-hint.md)）が
