...
- **OpenAI の structured outputs**: JSON schema で出力を制約する
- **Claude の tool use**: 関数シグネチャと型定義で引数を制約する
- **karasu の DSL**: アーキテクチャ記述専用の語彙で出力を制約する
...
- **以前**: 「論理/物理分離・drill-down・マルチファイル合成を持つテキストベース・アーキテクチャモデリングツール」
