...
`.krs` のテキスト形式は、変更が意味のある局所的な diff を生むように設計されている。
これは git と相性を良くするためだけではなく、
**あらゆる永続化レイヤー**（git、ブラウザデモの OPFS、将来の履歴機構）で
「何が変わったか」をユーザーに還元できるようにするためである。具体的には:
...
Git ネイティブなワークフローはこの性質から自然に導かれるが、目標自体は git に限定されない。
この目標の UX 上の帰結として「2 つの `.krs` をグラフィカルに diff 表示する」機能が考えられる（#650 参照）。

#### テキストがすべての authoring パスにおける唯一の source of truth

ユーザーが `.krs` を手書きするか、`translate` で生成するか、Chat パネル経由で作成するかに関わらず、
最終状態は常に `.krs` テキストである。どの authoring 経路もテキストを迂回しない。
これによりすべての入口が合成可能になる:
Chat の出力を人間が手で編集でき、逆に手書きしたモデルを Chat で洗練できる。
どの経路で辿り着いたかに関わらず、一貫したメンタルモデルが成立する。

この目標はさらに重要な副次効果を持つ:
**karasu の DSL は AI 支援モデリングのための制約付き中間言語として機能する**。
汎用 LLM にアーキテクチャを描かせると出力の形式と抽象度が不安定になるが、
karasu の DSL を挟むと出力は karasu の表現力の範囲に制約される。
...
#### グラフィカル / WYSIWYG な authoring は提供しない

キャンバス上でノードをドラッグしてモデルを作成するモードは目標に含まれない。
authoring は常にテキストを介して行う — ユーザーが入力するか、`translate` が生成するか、Chat が生成するか。
この非目標は Chat パネルの位置づけも明確にする:
