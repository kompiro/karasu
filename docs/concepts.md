...
#### 図はテキストから生成される — すべての入力経路の source of truth はテキスト

karasu では、図はテキスト（`.krs`）を記述することで生成される。
ユーザーが手書きするか、`translate` が既存コードから生成するか、Chat が生成するか —
どの経路であっても、最終的な成果物は `.krs` テキストであり、そこから図がレンダリングされる。
テキストを迂回する経路は存在しない。
これによりすべての入口が合成可能になる:
