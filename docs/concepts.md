...
| `shared-infra-fan-in` (info) | ≥ 2 services depend on the same `database` / `queue` / `storage` in one system (`[external]` and role-tagged (`[index]` / `[cache]` / `[analytics]`) stores excluded) | Microservices Database-per-Service treats a shared store as a smell — keyed on actual sharing, independent of how many files declared the store |
| `cross-domain-store-access` (info) | A usecase in one domain reads/writes an infra leaf owned by a *different* domain in one system (ownership derived from `entity` mappings; keyed per leaf; `[external]` and role-tagged stores excluded) | Bounded-context / DDD treats reaching into another domain's store as a boundary smell — legitimate under a shared kernel or during migration. Orthogonal to `shared-infra-fan-in`: ownership crossing vs service-count sharing |
