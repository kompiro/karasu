...
| `cross-domain-store-access` (info) | A usecase in one domain reads/writes an infra leaf owned by a *different* domain in one system (ownership derived from `entity` mappings; keyed per leaf; `[external]` / `[index]` excluded) | Bounded-context / DDD treats reaching into another domain's store as a boundary smell — legitimate under a shared kernel or during migration. Orthogonal to `shared-infra-fan-in`: ownership crossing vs service-count sharing |
