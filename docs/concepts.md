...
この目標は、karasu が何を扱うかの範囲を同時に明確にする:
karasu が扱うのは **ゆっくり変化する構造的な文脈** —
システムがどう組まれ、どこで動き、誰が所有するか — であり、
ミリ秒単位の呼び出しシーケンス、行単位の実装詳細、秒単位の稼働状態は、
karasu が答えるべき問いの粒度ではない。
後述の非目標（シーケンス・DB スキーマ・ランタイムメトリクス・コード生成）はこの原則の具体化であり、
「構造ではないもの」「ゆっくりではないもの」を切り出している。
