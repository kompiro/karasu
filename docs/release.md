...
5. 認証は **GitHub OIDC（Trusted Publishing）** — `release.yml` の `id-token: write` を npm が短命クレデンシャルに交換する。`NPM_TOKEN` は不要（保持しない）。provenance は trusted publishing で**自動付与**される（`--provenance` 不要）。要件は npm >= 11.5.1 / Node >= 22.14.0 で、`release.yml` が pin する Node 24（npm 11.17+ を同梱）が満たす。npm を別途アップグレードするステップは持たない（[ADR-2397](adr/2397-node-24-baseline.md)）。
