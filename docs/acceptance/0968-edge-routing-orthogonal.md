...
- [ ] EC Platform の例を本番 app（https://karasu.kompiro.dev/）で開き、`Admin → ECSite` のエッジが目視で MobileApp カードを横断していないこと、エッジ全体が L 字に折れて見やすくなっていることを確認する
  > 🧑 Manual — 本番 app（https://karasu.kompiro.dev/）で `examples/ja/ec-platform/02.5-clients.krs` を読み込み、視覚的に確認する。
