...
- **対象ファイル**:
  ...
  - `packages/app/src/utils/diagnostic-location.ts` / `packages/app/src/components/PreviewPane.tsx`
...
- [x] TC-C5: エントリ自身の位置はユーザーが打った綴りで印字し（相対形・symlink 越しでも同一と判定する）、ほかのファイルは作業ディレクトリからの相対パスで印字する
  ...
  > ✅ Automated — `packages/cli/src/compile-system-view.test.ts` › formatDiagLoc › keeps the user's spelling when the file is the entry reached another way / folds a relative entry onto its absolute form without touching the disk / names any other file relative to the working directory (TC-C5)
...
- [x] TC-D3: プロジェクトルートの外にあるファイルや、ルートと接頭辞だけ共有する兄弟ディレクトリは短縮しない
  ...
  > ✅ Automated — `packages/app/src/utils/diagnostic-location.test.ts` › diagnosticLocationLabel (#2715) › shows the full path when the file is not under the project root / does not shorten a sibling directory that merely shares the root's prefix (TC-D3)
