...
- **非常に幅広いラベルの best-effort 限界**: ラベル幅が周辺の空きより大きい密なクラスタでは、垂直方向 nudge の探索上限（既定 6 ステップ ≈ 90px）内で完全に clear できないことがある。その場合は最小コスト位置に置く（貫通を増やさないことは保証するが 0 は保証しない）。author は `label-position` / `label-offset`（ADR-1184）で明示的に逃がせる。
