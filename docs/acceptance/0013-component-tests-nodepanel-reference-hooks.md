...
Medium-priority tests identified in [ADR-9012](../adr/9012-app-testing-strategy.md).
...
### ReferenceContent / ReferenceWindow

The reference moved from an in-app modal panel to its own pop-out window
(#1548) so it can stay open beside the editor; `ReferenceContent` is the
tab body and `ReferenceWindow` is the `?reference`-mode shell.

| # | Criterion | How to verify | Manual? |
|---|-----------|---------------|:-------:|
| 6 | Clicking a tab renders that tab's content and hides other tabs' content | `ReferenceContent.test.tsx` | |
| 7 | Per-view content (system / deploy / org) renders the right kinds, samples, and selectors | `ReferenceContent.test.tsx` | |
| 8 | `ReferenceWindow` seeds the view selector from the `view` query param and switching it updates the content | `ReferenceContent.test.tsx` | |
| 9 | Copy button shows "Copied!" immediately after click and reverts to "Copy" after 2 seconds | `ReferenceContent.test.tsx` | |
