...
- **対象ファイル**:
  ...
  - `docs/adr/1829-adr-permalink-convention.md`（最初の dogfood `permalink:`）／ `.claude/rules/adr.md` / `docs/spec/permalink.md`（+ `.ja`）
...
- [x] AT-A: `pnpm adr:check-permalinks` が現行 repo で pass する（ADR-1829 の dogfood `#krs-system-Gateway` が解決）
