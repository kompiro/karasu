...
- [ ] AT-E: `examples/ja/multi-file-system/index.krs` を `karasu render` した SVG に、4 ファイル全てに宣言された全ノード・全 deploy node・全 team が現れる
  > ✅ Automated — `import-resolver.test.ts` の `"S2 + S4: end-to-end on the examples/ja/multi-file-system fixture"` ケース（実ファイルをドライブ）

- [ ] AT-F（manual）: App / VS Code 拡張で `examples/ja/multi-file-system/index.krs` を開き、`label` が「ブログプラットフォームデモ」になっていること、reader.krs / editor.krs を直接開くと各々の `label` (`Reader slice` / `Editor slice`) が表示されることを目視確認する（root-entry 優先と WYSIWYG メンタルモデルの一致）
...
- ADR: `docs/adr/1381-multi-file-import-semantics.md`
- 仕様化 PR: #1383 (merged)
- 実装 PR: 本 PR
- AT-H（S6 警告）実装: #1569
