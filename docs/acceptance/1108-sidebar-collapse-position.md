...
- [x] AT-A: `EditArea` に `sidebarContent` を渡すと collapse トグルボタンが描画される（既存テストで担保）
  > ✅ Automated — `EditArea.test.tsx` › `renders the activity bar Files toggle when sidebarContent is provided`

- [x] AT-B: トグルボタン押下で `.sidebar-collapsed` クラスが付与され、再押下で外れる（既存テストで担保）
  > ✅ Automated — `EditArea.test.tsx` › `activity bar button toggles the sidebar visibility` / `collapses and re-expands the sidebar when the active view button is re-clicked`

- [x] AT-C: `previewFocused` のときはトグルボタンが描画されない（既存テストで担保）
  > ✅ Automated — `EditArea.test.tsx` › `hides the activity bar when previewFocused is true`
...
- 旧 AT-E（Files ボタンでパネルが開閉する）と旧 AT-I（リロード後に幅が復元される）は削除した（[#2387](https://github.com/kompiro/karasu/issues/2387)）。前者は `EditArea.test.tsx` › `activity bar button toggles the sidebar visibility`、後者は `usePersistedPanelWidth.test.ts` › `seeds from a persisted value when present` が判定している。残した AT-G / AT-H は resize handle の**ジェスチャ**そのものを見る項目で、hook は fence されているが handle への配線を駆動するテストが無いため実機に残る。欠番の letter は既存の参照を保つためそのままにしている。
