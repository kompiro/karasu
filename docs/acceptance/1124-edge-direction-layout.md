...
- **関連 Design Doc**: [`docs/adr/9019-edge-direction-style.md`](../adr/9019-edge-direction-style.md)、[`docs/adr/1076-gui-driven-style-editing.md`](../adr/1076-gui-driven-style-editing.md)
...
- [ ] AT-F（manual）: 実際の Preview で edge を右クリック → Direction ▸ Up を選び、対象エッジが上下に flip して描画されることを確認する。drill-down view と top-level system view どちらでも有効
  > 🧑 Manual — 本番 app（https://karasu.kompiro.dev/）を開き、`examples/ja/getting-started/index.krs` の service 内 domain edge と top-level の user → service edge の両方で配置変化を目視
