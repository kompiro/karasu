...
- [x] grouped でも全ノードがちょうど一度描かれる（TPL-1738 の全域性）
...
- [x] `assignForcedSystemLayers` / `systemTier` の既定ビュー挙動は無改変（ADR-1724 の tier 体系は既定ビューで不変）
...
- [ ] cross-team edge が失われず両端点に接続している（TPL-1738）
