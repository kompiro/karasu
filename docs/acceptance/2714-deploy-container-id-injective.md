...
- [x] AT-C: quoted 形を構成する文字（`"` / `\`）は escape される

  > ✅ Automated — `packages/core/src/parser/node-path.test.ts` › nodePathRefId (#2714) › escapes the characters the quoted form is built from
...
- [x] AT-E: Issue のモデル（`Shop.Api` / `Admin.Api` / `"Shop.Api"`）で 3 つのコンテナが相異なる id を持ち、各コンテナが自分の unit を持つ
