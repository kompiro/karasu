...
- 既存 `packages/vscode-e2e/tests/webview/at-0038-cmd-click-hint.test.ts`
  に新しい `it()` を 1 つ追加: `TextEditor.moveCursor(2, 11)` で `OrderService`
  ...
  - AT-0038 のテストファイルを共有する理由: ExTester ハーネスの
    "File: Open File..." simple-dialog はセッション内の 2 回目以降の
    open で stall するため、テストファイルを分けると後続 suite が
    巻き添えで落ちる。AT-0037-9 は AT-0038 と同じ fixture / 同じ before
    で完結するので、suite 単位の `before` を共有するのが最も安価。
  - describe 名は `AT-0037-9 / AT-0038 (WebView) — bidirectional editor ↔ SVG preview`
    に拡張。
...
> ✅ Automated — `packages/vscode-e2e/tests/webview/at-0038-cmd-click-hint.test.ts` › `AT-0037-9: editor cursor on a node identifier highlights the matching SVG node` (CI は `vscode-webview-e2e` ラベル opt-in、xvfb 必要のため required check には昇格しない)
