...
| `shared-infra-fan-in` (info) | 同一 system 内で同じ `database` / `queue` / `storage` を 2 つ以上の service が参照する（`[external]` ストアは除外） | マイクロサービスの Database-per-Service は共有 store を smell とする — 何ファイルで宣言したかに依らず、実際の共有で判定する |
