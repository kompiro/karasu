...
3. `AppShell.tsx`:
   ...
   - `onSelectNode` → `dispatch({ type: "SET_HIGHLIGHTED_NODE", nodeId })`。
...
5. AT: `docs/acceptance/` に新規ファイル。TC は:
   ...
   - Outline ノード選択でプレビューの該当ノードがハイライトされる。
...
- Outline からのノード編集（CRUD）は今回行わない。
- Outline のノードを「クリックでプレビューの該当 viewPath へ drill-down する」
  か「ハイライトのみか」は、まずハイライトのみで実装する（Issue の受け入れ
  条件どおり）。drill-down は将来の拡張余地として残す。
