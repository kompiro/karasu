...
### ノード選択時の挙動 — シングルクリック=ハイライト / ダブルクリック=drill-down

VS Code の Outline に倣い、操作を 2 段階に分ける:

- **シングルクリック** — `highlightedNodeId` を当てるだけ（`viewPath` は変えない）。
  現 `viewPath` に描画されていれば preview がハイライトされる。
- **ダブルクリック** — preview を drill-down してそのノードを露出させ、ハイライト
  する。

ハイライトは `PreviewPane.tsx` の `useEffect` が現 SVG を `querySelector` する
だけの視覚効果なので、現 `viewPath` に無いノードはシングルクリックでは反応しない。
ダブルクリックの drill-down がそれを解決する。

drill-down 先の `viewPath` 解決:

- `nodeMetadata.viewPath` は `service` / `domain` / top-level infra
  （`database` / `queue` / `storage`）とその子（`table` / `queue-item` /
  `bucket`）にのみ付与される（`buildNodePathIndex`）。これらは「中に潜れる」
  ノードなので、自身の `viewPath` へ drill-in する。
- `viewPath` を持たない leaf（`usecase` / `resource` / `user` / `client` /
  `system`）は潜る先の sub-diagram を持たない。Outline は AST ツリーを持つので
  選択ノードの**祖先チェーン**を辿り、`viewPath` を持つ最も近い祖先の
  `viewPath` へ移動する（その祖先に潜れば leaf が子として描画される）。
  祖先にも無ければ root（`[]`）へ。
...
2. 新規 `packages/app/src/components/OutlineView.tsx`:
   - props: `systems: SystemNode[]`、`highlightedNodeId: string | null`、
     `onSelectNode`（シングルクリック）、`onActivateNode`（ダブルクリック、
     祖先 id 配列を渡す）。
   - `SystemNode` とその `children` を再帰描画する presentational ツリー。
     `FileTreeView` の構造に倣う。再帰で祖先 id 配列を引き回す。
   - シングルクリックで `onSelectNode(node.id)`、ダブルクリックで
     `onActivateNode(node.id, ancestorIds)`。`highlightedNodeId` と一致する
     item に selected スタイルを当てる。
3. `AppShell.tsx`:
   - `OutlineView` を生成し、`views.system.resolvedSystems` を渡す。
   - `onSelectNode`（シングル）: system ビューでなければ
     `SET_ACTIVE_VIEW`、そうでなければ `SET_HIGHLIGHTED_NODE`。移動なし。
   - `onActivateNode`（ダブル）: 上記のハイライトに加え、`[nodeId,
     ...ancestorIds 逆順]` を辿り `nodeMetadata.viewPath` を持つ最初の id の
     `viewPath` へ `navigateViewPath`。無ければ `[]`。
