...
**案1 を採用する（推奨）** — 現状の 7 断片 output は authored form ではなく、
...
**案A を採用する（推奨）** — #2175 が facet selector を migration target と
...
census という証拠と再スコープの決定が同じ ADR に並ぶのが最も追いやすい。
...
1. **論点 1 の採否**: 案1（stitching 全展開）で確定してよいか。freeze の
   「bug fix として読む」解釈を ADR に記録する前提。
2. **論点 2 の採否**: 案A（facet 書き換え）で確定してよいか。とくに
   「reading confidence は lifecycle 偽装に当たらない」という境界の引き方に
   同意できるか。
3. **論点 3 の記録場所**: 昇格 ADR に載せる（推奨）でよいか、それとも閉鎖時
   ADR まで roadmap 編集を待たせるか。
