...
#### A-2: from が自明なすべてのブロック（採用案）

`service`、`domain`、`usecase`、`resource` で `-> Target` を許容する。
...
usecase PlaceOrder {
  -> ValidateStock                  // PlaceOrder -> ValidateStock
}
```

**メリット:** 一貫した記法。冗長性を全体的に解消
**デメリット:** 変更範囲がやや広がる（ただしパーサーの変更箇所は1か所）
...
**適用範囲:** `system` 以外の全ブロック（`service`、`domain`、`usecase`、`resource`）。
`system` ブロック内のエッジは `from` が複数の service のいずれかを指すため、
この制約は `system` には適用しない。
...
| 軸 | A-1 domain 限定 | A-2 from 自明な全ブロック |
|----|----------------|------------------------|
| 一貫性 | △ | ◎ |
| 実装コスト | ○ | ○（同じ変更箇所） |
| ユーザー体験 | △ | ◎ |
...
**A-2（from が自明なすべてのブロック）を採用する。**

- `service`、`domain`、`usecase`、`resource` で `-> Target` / `--> Target` を許容
