...
- パーサーは変更後の新構文のみをサポートする
- label はあくまで表示用の文字列であり、参照・照合には使わない
- `system` を含むすべてのノード種別（system, service, domain, usecase, resource, user）が対象
- `system` ブロックは引き続き子ノードを持てる
- 現時点ではリリース前のため移行ガイドは不要（将来リリース後の構文変更時には自動変換 CLI を提供する）
...
// 変更後（label が省略された場合は id をそのまま表示名として使用）
el("text", {}, escapeXml(node.label ?? node.id))
```

> **方針決定**: label 省略時は id をそのまま表示する（CamelCase 変換等は行わない）。
> ユーザーは id 命名で可読性を考慮する（例: `ECommerce` → そのまま "ECommerce" と表示）。
...
| 論点 | 決定 |
|------|------|
| label 省略時の表示 | id をそのまま表示（変換なし） |
| system ノードの扱い | id 必須化の対象に含める |
| 移行ガイド | 現時点（リリース前）は不要。将来リリース後に構文変更が必要になった場合は自動変換 CLI を提供する |

## 未解決の問い

1. **エッジの inline label** — 現在エッジには `A -> B "label"` のような構文は存在しないが、今回の変更と合わせてエッジの label もプロパティ化するか？（別 Issue で扱う想定）
