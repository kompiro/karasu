...
`og:title` は krs から抽出した **プロジェクト名**を反映する。抽出は **best-effort**:

- payload を `decodeShare` → `compile(krs, { diagramType: "system" })` し、`systems[]` の先頭（synthetic な `__unassigned__` を除く最初の system）の `label ?? id` をタイトルに使う（例:「勤怠管理システム」「HRTool」）。
- decode / compile / 抽出のいずれかが失敗・該当なしなら **静的文言**（例:「karasu — shared architecture diagram」）にフォールバックする。
- **重要（後述の security と直結）**: 抽出した名前は krs 由来の任意文字列なので、`s` の charset validation では守られない。`og:title` の `content` に入れる前に **HTML escape を必ず行う**（escaping が load-bearing になる）。

これは「事前検証しない」方針と矛盾しない: タイトル抽出は失敗しても静的にフォールバックするだけで、**不正 payload でもページはエラーにならず OGP を出す**。`og:description` は v1 では静的（ビュー種別の一言程度）。
...
- **decode 由来の文字列は別扱い**: 動的 `og:title` は `decodeShare` 後の任意文字列で charset validation の外。属性に入れる前に **HTML escape を必須**とする（`&` `<` `>` `"` `'`）。抽出は try/catch の best-effort、失敗時は静的タイトル（事前検証はしない）。
- **タイトル抽出**: `decodeShare(s)` → `compile(krs, { diagramType: "system" })` の `systems[]` 先頭（synthetic `__unassigned__` を除く最初の system）の `label ?? id`。該当なし・例外時は静的フォールバック。
...
- **og:title**: krs からプロジェクト名を抽出して**動的化**する（best-effort、失敗時は静的）。抽出値は HTML escape 必須。
