...
### og:title / og:description（確定: 静的）

`og:title` / `og:description` は **静的文言**にする（例: title=「karasu — shared architecture diagram」）。krs からプロジェクト名を抽出して動的化すると、Function 内で parse が必要になり escaping 面も増える。v1 は静的。動的タイトルは後続。

## 現時点の方針

**query スキーム `/s?s=` + SPA への bounce + system 1 枚 OGP + 静的 title** を採用する。既存の `/render` PNG・`encodeShare`・`readSharedProjectFromHash` をすべて再利用し、新規描画も SPA 配線も持たずに最小実装で OGP unfurl を実現する。Share ダイアログは private / unfurlable の 2 リンクを **トレードオフを伝える文面付き**で提示し、oversize 時は private のみへフォールバックする。
...
- **payload validation（TPL-20260510-17）**: `s` が base64url 文字集合 `[A-Za-z0-9_-]+` に厳密一致するかを検証。外れたら 400。base64url には `"` `<` `>` `&` `'` が含まれないため、これで HTML 属性 / JS 文字列への injection を構造的に封じる。さらに属性へ埋める箇所は HTML escape も併用（defense in depth）。
...
- 動的 og:title（krs からプロジェクト名抽出）は後続。
- payload を decode して描画可能性を事前検証するか: v1 では行わない（不正 payload は `/render` 側が適切な status を返し、unfurl が空になるだけ）。
