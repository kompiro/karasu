...
- **description サマリ**: `description` の先頭1行目のみ表示。ノード幅を超える場合は省略記号 `…` で切り詰め。description がノードの幅を広げることはない（ラベル幅がボックス幅を決定する）
- **メタ行**: link 数（`🔗2`）と team 名を小さいフォントで表示。両方ある場合は link を左寄せ、team を右寄せで配置。情報がなければ行自体を省略
...
ノードの幅:
  ...
  ※ description サマリの幅はノード幅の決定に影響しない。
...
5. レンダリング時に、ノード幅に収まらない場合はさらに `…` で切り詰める
...
service および domain ノードに表示される。メタ行内に小さいフォントで描画する。
...
- **Markdown レンダリングライブラリ**: `marked` を採用。XSS 対策として `dompurify` を併用
- **イベント伝播**: NodeDetailPanel は `onClick`, `onMouseDown`, `onMouseUp`, `onWheel` で `stopPropagation` し、背面のパン/ズーム操作と干渉しない
- **プロトタイプ汚染防止**: タグベース shape 検索に `Object.hasOwn()` を使用
- **link ラベル未指定時**: URL をフォールバック表示する
