...
5. **repair 再開は quota を消費するか。** $0.1 の再開に 1 枠は釣り合わないが、無料にすると「失敗させてから repair を叩く」経路が枠を回避できてしまう。回数上限（同一 sha につき n 回まで）で縛る案もある
6. **repair 再開の入口をどうするか。** 専用エンドポイント（`POST /:owner/:repo/repair`）にするか、既存の generate にフラグ（`{resume: true}`）を足すか。前者は意味が明確、後者は面が増えない
7. **失敗文書の TTL 24 時間で足りるか。** 再開の入力になるなら、失敗に気付いてから叩くまでの時間がそのまま制限になる。問い 1（run 状態の TTL）と同じ判断に属する
