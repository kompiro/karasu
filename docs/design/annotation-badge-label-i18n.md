...
```ts
// EmptyStateLabels とは別の独立オプションとして追加する（既存 API 無変更）。
// @new も対称性のため含める（現状 en/ja とも "NEW" だが、将来訳を変えても
// API 変更が不要になる）。省略時は reference-data の en ラベルを使用。
...
レビューで確定した詳細（2026-06-10）:

- **API の形**: `AnnotationBadgeLabels` は `EmptyStateLabels` とは **別の
  独立オプション** とする。既存 API 無変更で増分が素直。`CoreLabels` への
  統合は注入オプションが 3 つ以上に増えた時点で再検討
- **`@new` の扱い**: 注入可能キーに **含める**。4 アノテーションを対称に
  扱い、将来 ja 訳を変えるときに API 変更を不要にする
- **CLI / VS Code**: 今回は **スコープ外**（デフォルト en）。注入口だけ
  整備し、`karasu render --locale ja` と翻訳テーブルの置き場所は別 Issue で
  扱う
