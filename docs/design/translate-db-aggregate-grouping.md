...
検討はしたが、対象を広げるほど「たまたま同名の列」を誤って親リンクと解釈するリスクが上がる。`_id` と `_code` は業界的にほぼ親テーブル参照を意味する慣習が固まっているので、まずこの 2 つに限定する。需要が出てきたら拡張する（Soft FK のリスト化は一元管理されているので増やすコストは低い）。
