...
- **out of scope（別 Issue として後続フェーズで扱う）**:
  - `/<owner>/<repo>` GitHub resolver（Phase 2）→ 独立 Issue
  - in-site editor + repo への `.krs`/画像 PR 還元ループ（Phase 3）→ 独立 Issue
...
3. **app の URL ソース入口**: `MemoryModeApp` を fragment デコード結果で初期化する経路を追加する。共有 URL の生成・コピー UX は後述「UI: Share ボタンと共有ダイアログ」を参照。
...
9. 後続フェーズは本ドキュメントに抱えず、`/<owner>/<repo>` resolver（Phase 2）と editor + PR 還元ループ（Phase 3）を **それぞれ独立 Issue** として起票する。
...
- **Phase 2 / 3**: 本ドキュメントには抱えず、`/<owner>/<repo>` resolver と editor + PR 還元ループを**それぞれ独立 Issue** として後続で扱う。Phase 1 は karasu app 内で完結する。
