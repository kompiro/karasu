...
| **PR 3 — 静的 SVG エンドポイント** | Cloudflare Pages Function（`packages/app/functions/render.ts`）が `GET /render?s=<payload>&view=…` で共有 payload を SVG にレンダリング。ロジックは `packages/app/src/render/share-render.ts`（テスト済み）。入力は inline-share の fragment と違い **query**（サーバに届く）。**SVG 先行、PNG（OGP 用 `@resvg/resvg-wasm`）は別 PR**。README 埋め込み用 | PR 1 |
