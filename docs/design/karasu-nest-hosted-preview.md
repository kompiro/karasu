...
- **out of scope（MVP 外。アイデアとして GitHub Discussions に記録）**:
  - `/<owner>/<repo>` GitHub resolver（Phase 2）→ Discussion (Ideas)
  - in-site editor + repo への `.krs`/画像 PR 還元ループ（Phase 3）→ Discussion (Ideas)
...
9. 後続フェーズは本ドキュメントに抱えず、`/<owner>/<repo>` resolver（Phase 2）と editor + PR 還元ループ（Phase 3）を **GitHub Discussions（Ideas カテゴリ）にアイデアとして記録**する。MVP 外の未確定構想を backlog 化せず温めておくのが目的で、機運が高まった時点で Issue 化する。
