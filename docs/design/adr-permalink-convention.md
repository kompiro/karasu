...
- **日付**: 2026-06-29
- **ステータス**: 検討中
- **改訂 (2026-07-01)**: 決定を **B-4 → B-3** に改訂。生の `/s?s=` payload は数 KB に
  及びうるため ADR frontmatter にインラインしない。karasu（L2）は **taka 短縮リンク +
  必須 `source`（in-repo `.krs`）** を採用する。記録（SoT）は in-repo `.krs`、permalink
  はその pointer。`source` を必須にすることで、taka が消えても構造を repo から復元でき、
  shortener を単一障害点にしない。point-in-time の厳密な immutability は #1828（ref-pin）
  待ち。以下の本文中「正は payload（B-4）」の記述はこの改訂で置き換わる（履歴として残す）。
- **関連**:
...
| **proactive TPL（本 PR で新規、改訂 B-3）** | 「ADR permalink は record ではなく pointer。shortener や URL の形が消えても構造が残るよう、in-repo `.krs` の `source` を必須で記録する」。L1 guide（`docs/guide/`）/ L2 rules を派生元 spec として相互リンク |

## 現時点の方針

**規約を 2 層に分け、L1（portable guide）を baseline、L2（adr-tools 実装）をその reference 実装とする。記録（SoT）は in-repo `.krs`、permalink はその pointer。karasu（L2）は taka 短縮リンク + 必須 `source`（in-repo `.krs`）を採用する（B-3。改訂 2026-07-01）。`source` を必須にすることで、taka が消えても構造を repo から復元でき、shortener を単一障害点にしない。生の `/s?s=` payload は数 KB に及びうるため frontmatter にインラインしない。本文のクリック用サマリは adr-tools が生成する。**

> 改訂前の方針（B-4: 正は自己完結 `/s?s=` payload、短縮は任意別名）は下の理由・options 分析に履歴として残す。数 KB の payload を ADR に載せたくないというユーザー判断で B-3 に改訂した。
...
- **記録は in-repo `.krs`（SoT）、permalink はその pointer**。リンクの隣に **`source`（in-repo `.krs` への参照）を必須で記録**し、リンク単体を記録にしない（リンクを単一障害点にしない）。
- 短縮する場合は **`/s?s=`（query）を宛先に短縮**する（`#s=` fragment は server に届かず unfurl が死ぬので不可 — #1786 契約 1）。短縮は taka でも他サービスでもよい。短縮しない選択（長い URL をそのまま貼る）も可。生の payload は数 KB になりうるため ADR にインラインしなくてよい（`source` が復元を担う）。
- deep permalink（#1833）なら `source` に対象要素 id を添える（例 `payments.krs#krs-system-payment-api`）。identity は `id`、`label` ではない。
...
frontmatter に `short`（taka）と `source`（必須）を持つ。`view` は任意（改訂 B-3）:

```yaml
permalink:
  - short:  https://taka.kompiro.dev/TkrZQG    # taka 短縮リンク（クリック用 pointer）
    source: examples/keystone/payments.krs     # 必須: in-repo .krs（記録・復元元）
    view:   system                             # 任意: 既定 view
```

生の `/s?s=` payload は frontmatter に載せない（数 KB になりうる）。復元は `source`
から。真の ref-pin は #1828。
...
1. **L1 guide を公開ドキュメントとして文書化** — `docs/guide/adr-permalinks.md`（＋ i18n ポリシーに従い `.ja.md` 兄弟）を新設し、上記 L1 content ルール（記録は `source`・permalink は pointer・短縮の宛先は `/s?s=`・trust note）を規定。`packages/docs-site/scripts/lib/site-map.ts` の `PUBLISHED_DOCS` に登録して docs-site で公開する（ユーザー向け）。
...
3. **#1830 / adr-tools への引き継ぎ** — frontmatter `permalink:` の検証（必須 `source` `.krs` が実在するか・`short` が解決するか）と本文サマリの生成は #1830 / `@kompiro/adr-tools` に委ねる。本 Doc は「検証・生成可能な記述形」を確定するところまで。
4. 既存の代表 ADR 1 本（例: keystone 系、または ADR-20260626-04）に L2 を適用したサンプルを示す。
5. **proactive TPL を 1 件同梱**（CLAUDE.md の「spec/concepts/guide 新規節には proactive TPL を最低 1 件同梱」規約）— 観点:「ADR permalink は record ではなく pointer。shortener や URL の形が消えても構造が残るよう、in-repo `.krs` の `source` を必須で記録する」。L1 guide / L2 rules を派生元 spec として back-ref する。
...
- **記録は in-repo `.krs`・permalink は pointer（改訂 B-3、2026-07-01）**。karasu（L2）は taka 短縮リンク + **必須 `source`** を採用する。生の payload は数 KB になりうるため frontmatter にインラインしない。`source` 必須で shortener を単一障害点にしない。immutability の厳密化（ref-pin）は #1828。
  - *改訂前（B-4）*: 「正は自己完結 `/s?s=` payload、短縮は任意別名」。数 KB payload を ADR に載せたくないため B-3 に改訂（履歴として options 分析を残す）。
- **L2 は frontmatter `permalink:`（`short` + 必須 `source`）が記録、本文サマリは adr-tools が生成**（二重メンテなし）。
...
- **proactive TPL は 1 件に集約** — 観点「ADR permalink は record ではなく pointer。shortener や URL の形が消えても構造が残るよう、in-repo `.krs` の `source` を必須で記録する」を同 PR で起こす。

## 未解決の問い / 決めないこと

1. **frontmatter `permalink:` の検証強度（#1830 へ申し送り）。** 必須 `source` の `.krs` 実在チェックは必須として、`short` の 302 解決チェックを CI で常時叩くか（外部 taka への依存をテストに持ち込む是非）。本 Doc では「申し送り」に留める。
