...
7. proactive TPL: `iconNameForNode` と `ICON_THEME_STYLE_SOURCE` という
   「同一語彙の二重表現」が片方だけ更新されて drift する観点を proactive TPL
   として同 PR で起こす（下記「Related TPLs」参照）。
...
- [TPL-20260510-06](../test-perspectives/TPL-20260510-06-display-mode-cross-surface.md)
  ...
  共有させることでこの観点に応える。
- proactive TPL（同 PR で起票予定）— `iconNameForNode` と
  `ICON_THEME_STYLE_SOURCE` のように同一語彙を CSS と関数の 2 表現で持つ場合、
  片方だけ更新されると Outline と Icon Mode が静かに drift する。新しい kind /
  variant を `ICON_THEME_STYLE_SOURCE` に足したら `iconNameForNode` 側にも
  足す（逆も同様）ことを保証する観点。

## 未解決の問い / 決めないこと

- proactive TPL を「同一語彙の二重表現の同期」という汎用観点で起こすか、
  「icon-theme の CSS / 関数二重表現」に限定した観点で起こすか。実装 PR で
  `test-perspective` スキルを使って確定する。
- 案1 のデメリットを将来的に解消するなら、`iconNameForNode` を真実源にして
  `ICON_THEME_STYLE_SOURCE` をそこから生成する案が考えられるが、本 Design
  Doc では扱わない（将来の別 Issue）。
