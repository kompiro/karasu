...
- **stroke-width**: 視覚的な「ぱっと見」の強弱を 1.5 / 2.0 px で区別する（A の利点）
- **label `W` / `R`**: 全 edge にテキストマーカーを乗せ、移行分析で「太いだけかどうか曖昧」を排除する（D の利点）。read も明示することで「label 無し = 何の情報か」という曖昧性が消え、図全体が一貫したフォーマットになる
- 軸が直交するので 4 通り全て区別可能
...
2. **`view-extract.ts` `deriveUsecaseResourceNodes`** を拡張:
   ...
   - `label` を `"W"`（write）または `"R"`（read）で設定する（全 edge に必ず付与）
...
- **冗長エンコーディング** で a11y / 可読性が高まる。stroke-width で「ぱっと見の強弱」、label で「`R` / `W` の確証」を独立に提供する
- **両方ラベルを出す**ことで「label 無しは何の情報か（read / 未判定 / バグ）」という曖昧性が消え、フォーマットが全 edge で一貫する
- 「移行で危険な側（`W`）を強調する」という意味付けは、Issue #1061 の主目的（移行影響の可視化）と直結する。read 側にも `R` が乗ることで「この edge は意識的に read と判定された」という確認情報が得られる
...
- **label 文字列**: `"R"` / `"W"` の両方を全 edge に乗せる。i18n は不要（read/write は日本語環境でも通じる慣用語彙）。**確定**。
