...
| sync + write | solid | 3 | `W` |
| async + read | dashed | 1.5 | (なし) |
| async + write | dashed | 3 | `W` |
...
6. **ユーザー上書き可能性**:
   ...
   - label 文字列は AST レベルで上書きする手段が現状ないので、v1 では `"W"` 固定。i18n / カスタマイズは後続 issue で扱う
...
太さの差だけだと「他の太い edge（cyclic edge は 2.5、user 上書きで 2 になっている edge も別途あり得る）」と混同するリスクがあり、移行分析で確証が必要なシーンで弱い。`W` ラベルが乗ることで「太い理由は write だ」と一意に決まる。冗長エンコーディングのコストは v1 では低いので、片方だけにする理由が薄い。
...
- **label 文字列**: `"W"` / `(空)` で進めてよいか、`"R"` も明示的に出すか、日本語圏向けに `"書"` / `"読"` を選べるようにするか。**推奨: v1 は `"W"` のみ固定**（i18n / カスタマイズは後続 issue）。
- **width 比**: `read=1.5 / write=3`（2 倍）でよいか、控えめな `1.5 / 2.5`（cyclic と同じ）がよいか。**推奨: 1.5/3**（cyclic = 2.5 と区別できる方が安全）。
...
- **examples の追加**: 既存 `feature-samples/resource-operations.krs` を増強するか、新規サンプルを足すか。**推奨: 既存サンプル増強**（feature 1 件 1 ファイルの慣例と整合）。
