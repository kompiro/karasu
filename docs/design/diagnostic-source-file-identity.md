...
| 消費者 | 変更後 |
| --- | --- |
| CLI `formatDiagLoc` | `+1` をやめる。`loc.file` が無ければエントリ文書として扱う（**ユーザーが打った綴り**をそのまま使う。単一ファイル時の現状と同じ出力）。`loc.file` があってエントリと同じならやはり打った綴りを使い、違うなら cwd からの相対パスにする（同一判定は下記の正規化を経る） |
...
比較には両者を `fs.realpathSync`（throw したら `path.resolve` に退避）で畳んだ形を使い、
**表示にはユーザーが打った綴りを残す**。この「正規化して比べ、綴りは保つ」の分離が
`formatDiagLoc` の責務である。

正規化は**同期**でなければならない。`formatDiagLoc` は `string` を返し、呼び出し側は
その結果をそのまま `process.stderr.write` に渡している。非同期版を使うと
`formatDiagLoc` と全呼び出し側が `await` を要する形に変わる。
