...
- **タブ手動クリック時のハイライト**: 消えて正しい。`highlightNodeId` を省略した
  `SET_ACTIVE_VIEW` が `null` にリセットするため、既存動作がそのまま期待通りに機能する。

- **将来ビューで highlight が不要なケース**: `highlightNodeId` はオプションフィールドのため、
  ハンドラが渡さなければ自動的に `null` になる。ビュー追加側の裁量に委ねられており、
  アーキテクチャ変更は不要。

### 未解決

#### ブラウザバック/フォワード時のハイライト復元

現状、`highlightedNodeId` は URL に反映されないため、ブラウザバック/フォワードで
ハイライトが復元されない。（例: D ボタン → Deploy 図でハイライト → 別ビューに移動 →
バック → Deploy 図に戻るが**ハイライトなし**）

**クエリパラメータ案（`?highlightNodeId=ECommerce#krs-deploy`）の問題点:**
現アーキテクチャは hash 専用で `history.pushState` で hash のみを更新する。
`?highlightNodeId=ECommerce#krs-deploy` の状態から次のビューに `pushState` すると
`?highlightNodeId=ECommerce#krs-system-root` となり、クエリが他ビューに**漏れ続ける**。
これを防ぐには全ての hash 更新で `?highlightNodeId` も同時に管理する必要があり、
実装複雑度が上がる。

**hash 拡張案（`#krs-deploy:ECommerce`）:**
...
`buildHash` に `highlightNodeId?: string | null` を追加し、
`parseHash` がコロン以降を `highlightNodeId` として返す。
`popstate` ハンドラで `highlightNodeId` を取り出し `SET_ACTIVE_VIEW` に渡す。

**スコープの判断:**
ブラウザ履歴でのハイライト復元は「D ボタンクリック後すぐに戻るケース」で有益だが、
Issue #422 の本質（初回ナビゲーションでハイライトが全く効かない）とは別の機能追加。
→ **Issue #425** として切り出し済み。
