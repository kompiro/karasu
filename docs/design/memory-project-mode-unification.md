...
## 決定した方針：WarningPanel の配置

`WarningPanel` は `KarasuPreviewColumn` に**内包する**。

`activeView === "org"` で org warnings / logical warnings を切り替えるロジックが
3ビューモデルの中で完結するため、コンポーネントが自己完結する。
CSS の `preview-column` レイアウト（flex column の末尾）への調整は局所的で済む。

---

## SAMPLE_KRS と Getting Started プロジェクトのコンテンツ設計

### 方針

MemoryModeApp が3ビュー（System / Deploy / Org）をサポートするようになったとき、
初期コンテンツが System のみでは「Deploy / Org タブが空」に見える。
そのため、**SAMPLE_KRS を System + Deploy + Org を含む完全な例に更新する**。

ProjectModeApp の Getting Started プロジェクト（`DEFAULT_KRS`）も同様に更新する。
どちらも「3タブすべてが動く状態で届ける」ことを目的とする。

### 要件

- System ビュー：サービス・ドメイン・ユースケース・リソースが含まれる
- Deploy ビュー：System ビューのサービスに対応する `deploy` ブロック（コンテナ定義）がある
- Org ビュー：`organization` ブロックと `team` 定義がある
- クロスナビゲーション：Deploy のコンテナをクリックすると System ビューのノードがハイライトされる
- サンプルのドメインは学習しやすいシンプルな例とする（EC サイト程度の規模感）
...
deploy "..." {
  // containers referencing services by id
}
...
## ReferencePanel への Samples タブ追加

### 動機

ProjectModeApp では ReferencePanel（Syntax / Styles / Tags / Built-in Theme）が
構文リファレンスを提供している。しかし「実際にどう書けばよいか」の手がかりがなく、
初見ユーザーが3つのビューをどう使い分けるかイメージしにくい。

### 方針

ReferencePanel に **"Samples" タブを追加する**。
ここに system + deploy + org を含む完全なサンプル `.krs` を掲載し、
コピーボタンで丸ごとエディタに貼り付けられるようにする。

```
ReferencePanel タブ構成（追加後）
├── Syntax
├── Styles
├── Tags & Annotations
├── Built-in Theme
└── Samples          ← NEW
    └── 完全サンプル（system + deploy + org）+ Copy ボタン
```
...
| コンテンツ | 用途 | 内容 |
|-----------|------|------|
| `MemoryModeApp` の `SAMPLE_KRS` | 起動時にエディタに表示される初期コンテンツ | system + deploy + org の完全例 |
| `ProjectModeApp` の `DEFAULT_KRS` | Getting Started プロジェクトの初期ファイル | system + deploy + org の完全例 |
| ReferencePanel "Samples" タブ | どちらのモードでも参照できるコピー可能なサンプル | 同等の完全例（または解説付き） |

サンプル内容は共通化できる部分が多い。`@karasu/core` の `getReference()` に
サンプル文字列を追加して一元管理する方法も検討する。

### 実装ステップ（Samples タブ）

1. `@karasu/core` の `getReference()` 戻り値にサンプル文字列 `sampleKrs` を追加
2. `ReferencePanel` に `"samples"` タブを追加
3. `SamplesTab` コンポーネント：サンプルを `<pre>` 表示 + Copy ボタン（Built-in Theme タブと同パターン）

---

## リファクタリング全体の実装順序（更新版）

### フェーズ1: ProjectModeApp の3ビューモデル化

1. Reducer 状態の統一：`diagramType` + `viewKind` → `activeView: "system" | "deploy" | "org"`
2. `DiagramTabBar` のインターフェース更新：`current` + `viewKind` → `active: ActiveView`
3. `KarasuPreviewColumn` の抽出（`WarningPanel` を内包）

### フェーズ2: MemoryModeApp の移行

4. `InMemoryFileSystemProvider` の実装
5. `MemoryModeApp` を `useAppContext` + `KarasuPreviewColumn` に書き換え
6. `SAMPLE_KRS` を system + deploy + org を含む完全例に更新

### フェーズ3: コンテンツ整備

7. `DEFAULT_KRS`（Getting Started プロジェクト）を同等の完全例に更新
8. ReferencePanel に "Samples" タブを追加（`@karasu/core` の `getReference()` 拡張）
