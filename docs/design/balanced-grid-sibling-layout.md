...
この原則は本 PR で `docs/concepts.md` / `docs/concepts.ja.md` の scoped glance 節に
**「解像度の軸」として明文化した** — scoped glance はナビゲーションの深さだけでなく、
**単一ビューが一目で把握できる解像度・視覚的密度を保つこと**までを含む、という規定。
本 Design Doc はその概念をレイアウトで具現化する。
