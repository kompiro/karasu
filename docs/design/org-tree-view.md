...
- **ステータス**: 実装中
- **関連**:
  - [Issue #309](https://github.com/kompiro/karasu/issues/309) — 初期実装（完了）
  - [Issue #320](https://github.com/kompiro/karasu/issues/320) — スタイル対応（本ドキュメントで設計）
...
### Phase 1: 基本実装（Issue #309 — 完了）

1. **`src/renderer/org-tree-renderer.ts`**（新規）— 完了
2. **`src/renderer/org-tree-renderer.test.ts`**（新規）— 完了
3. **`src/index.ts`**（更新）— 完了
4. **`src/hooks/useOrgView.ts`**（更新）— 完了
5. **`src/components/KarasuPreviewColumn.tsx`**（更新）— 完了
6. **`docs/acceptance/0044-org-tree-view.md`**（新規）— 完了

### Phase 2: スタイル対応（Issue #320）

#### 設計方針

`.krs.style` のセレクタ `team` / `member` / `#NodeId` を Org Tree View にも適用する。
スタイル解決は既存の `resolveStyles()` がすでに `organizations` を受け取り
全チーム・メンバーの `ResolvedNodeStyle` を `styles.nodes` に格納するため、
**renderer 側に `ResolvedStyles` を渡すだけ**でスタイルカスケードが機能する。

#### スタイルプロパティのマッピング

| `ResolvedNodeStyle` フィールド | SVG 属性 | 対象 |
|---|---|---|
| `backgroundColor` | `fill` | チームカード・メンバーカードの背景 |
| `borderColor` | `stroke` | カードの枠線色 |
| `borderWidth` | `stroke-width` | カードの枠線幅 |
| `borderRadius` | `rx` | カードの角丸 |
| `color` | `fill`（テキスト） | チームラベル・メンバー名 |
| `fontFamily` | `font-family` | テキスト全般 |
| `fontSize` | `font-size` | チームラベル・メンバー名 |
| `fontWeight` | `font-weight` | チームラベル |

コネクタ（Bezier）は `ResolvedEdgeStyle.color` / `strokeWidth` を使用する。

#### `RenderOrgTreeOptions` の拡張

```typescript
export interface RenderOrgTreeOptions {
  forExport?: boolean;
  styles?: ResolvedStyles;   // ← 追加
}
```

`styles` が未指定の場合はハードコードされた定数にフォールバックする。

#### データフロー

```
_compileCore / _compileProjectCore
  └─ resolveStyles(systems, sheets, undefined, organizations)  // 既存
  └─ OrgCompileResult.styles = styles  // 新規追加

useOrgView (app)
  └─ state.styles = result.styles  // 新規追加
  └─ renderOrgTreeView(orgs, expandedIds, { styles })  // 新規追加
```

#### 変更ファイル（Phase 2）

1. **`packages/core/src/renderer/org-tree-renderer.ts`**
   - `RenderOrgTreeOptions` に `styles?: ResolvedStyles` を追加
   - `renderTreeTeamCard` / `renderTreeMemberCard` / `bezierConnector` に `ResolvedNodeStyle` / `ResolvedEdgeStyle` 引数を追加

2. **`packages/core/src/index.ts`**
   - `OrgCompileResult` に `styles: ResolvedStyles` を追加
   - `_compileCore` / `_compileProjectCore` の org ブランチで `styles` を返す

3. **`packages/app/src/hooks/useOrgView.ts`**
   - `OrgViewState` に `styles?: ResolvedStyles` を追加
   - `renderOrgTreeView` の呼び出しに `{ styles }` を渡す

4. **`docs/spec/style.md`**
   - `team` / `member` セレクタのドキュメントを追加
...
### Phase 2: `styles` を `RenderOrgTreeOptions` に含める vs 直接引数にする

直接引数にすると後方互換性が崩れる（既存のテスト・呼び出し側を全変更）。
オプションオブジェクトへの追加はオプショナルなので後方互換性を維持できる。→ オプションへの追加を採用。
