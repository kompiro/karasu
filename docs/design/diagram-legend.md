...
| 案 | 却下理由 |
|----|---------|
| 図 bbox の右下コーナー | 図が成長するとノードと重なるリスク。レイアウト依存。さらに app の pan（右クリックドラッグでビューが動く）操作で凡例が画面外に流れてしまう。|
...
> footer 帯のほうが overlap リスクなく純粋であることに加え、app の
> 右クリックドラッグによる pan 操作で凡例が画面外に流れる問題も避けられる。
> 本マージ時に footer に揃えた。
