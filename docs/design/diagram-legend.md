...
#### D-1: 単一の `legend` ブロックは全ビューに出る（採用候補・最小ケース）
...
**Cons**
- 「deploy では物理層特有の凡例を出したい」用途が後で出てきたら拡張要。

#### D-2: `legend system "..." {}` のようにビュー指定可

```krs
legend system "Owner team" { ... }
...
**Cons**
- v1 に乗せると複雑度が上がる。設計の余地は残す。

→ **D-1 を v1 として採用**。D-2 は後続 Issue で拡張可能な余地として残す。
   `legend "..." {}` は将来 `legend <view-scope>? "..." {}` に拡張する形を想定。
...
| D ビューごと | v1 は単一凡例を全ビュー、scope 拡張は後続 | 最小ケース優先 |
...
```ebnf
legend     ::= "legend" string-literal? "{" entry* "}"
...
1. **AST**: `KrsFile.legends: LegendBlock[]` を追加。
   `LegendBlock { title?: string; entries: LegendEntry[] }`
...
2. **Parser**: トップレベル `legend` キーワードを受理する分岐を `parser.ts`
   に追加。エントリは `swatch` / `ref` のサブパーサに分離。
3. **Renderer**: 各 SVG renderer (system / deploy / org) に
   `renderLegend(legends, styles): string` を呼び出す共通ヘルパを追加。
   `ref` エントリの色は `resolveStyles` の結果から `selector` を引くこと
   で取得（無解決ならスキップ + warning）。
...
- **Q1**: 凡例は drill-down 後（ネストされたビュー）でも出すべきか？
  → v1 はトップレベルビューのみ表示で良いと思うが、ユーザーに確認したい。
- **Q2**: `ref` の selector 解決順は最初の一致でよいか、それとも
  CSS specificity に従うか？
  → 解決済みスタイル（`resolveStyles` の出力）を引く方針なので
    自然に specificity 順になる。問いとしてはクローズ可。
