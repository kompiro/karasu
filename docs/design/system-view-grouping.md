...
- **文法変更ゼロ** — `organization` / `team` / `owns` は既存構文。`ownerIndex` は既に `layout()` に届いており、`collapseNodeList` と同じ場所で使える。
- **単一値** — `ownerIndex` が 1:1 なので、フレームの不変条件を最初から満たす。
- **意味がある** — Conway の法則により、チーム境界はしばしば実際に意味的なまとまりである。合成タグではなく実データで仮説を検証できる。

> **面をまたぐ判断**: karasu は論理・物理・組織の三面を分離する。論理ビュー（system view）に**組織の境界を描くのは面をまたぐ**行為であり、意識的な選択である。本 Doc ではこれを**採用**する — システム構成図に組織の境界が現れること自体に価値がある、という判断（#1822 のレビューで合意）。

### 検証内容

1. 密な system view を、境界フレーム **あり / なし** で描き分ける。
2. チーム単位で **collapse → ⊕ + 件数スタブ** に畳み、要素数が減ったときに読み取りやすくなるかを見る。
3. **cross-team edge**（チームをまたぐ線）が失われず、むしろ際立つかを見る。
4. 判定は「要素数が減ったか」だけでなく、**「構造の理解が速くなったか」**で行う。畳んだ結果として関係が読めなくなるなら仮説は偽。

### 被験体

現状 `examples/` の最大は `examples/en/hato/index.krs` の 15 ノードで、密度問題の実証には小さい。#1816 / #1820 が定める **karasu-nest corpus（実 OSS アーキテクチャを書き起こしたもの）** を被験体に用いる。gate の evidence 要件とも一致する。
...
- 描画は `krs-cat-frame` / `renderContainer` を再利用し、`collapseNodeList` の畳み込みをそのまま適用できる。
...
- **P1 の判定基準を何に置くか** — 「要素数が減った」ではなく「構造の理解が速くなった」をどう観測するか。karasu-nest corpus での主観評価に留めるか、指標を立てるか。
