...
- ~~**scoped boundary をどう指すか**~~ → 実装時に決めた。**修飾なしの `boundary#pci` は全スコープの同 id に一致する**（スコープ修飾を持たない = 修飾を問わない、という素直な読み）。スコープ限定が必要になったら、修飾なしの意味を変えずに修飾形を足せる。解釈は `docs/spec/style.md`（+ja）に明記済み。実装は group id を `displayGroupId` で著者向けの id に正規化してから引く。最初の実装は group id のまま引いており、scoped boundary で無言の無効果になっていた（レビューで検出。TPL-1666 / TPL-1352 の形）。
