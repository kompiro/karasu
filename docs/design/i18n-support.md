...
| UI 言語セレクタ（ツールバー） | ❌ 未実装 | `setLocale()` は公開されているが UI から呼び出されていない |
...
- ツールバーに言語セレクタを配置し、ユーザーが明示的に切り替えられる（UI はまだ）
...
**サブ PR 候補**（独立してマージ可能）:
- C1: ツールバー + 言語セレクタ本体（これで UI からの切り替えが可能になる）
- C2: Settings pane
