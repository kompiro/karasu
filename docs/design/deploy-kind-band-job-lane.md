...
- **out of scope**: deploy 幅（ADR-20260408-02 済）、kind color/style-system 統合（#30）、store/infra 帯の正式コード化（候補 B、本設計では見送り）。
...
**案 C を採用する** — job-only container を DAG から外して専用帯に集約する。compute/store の依存 DAG はそのまま。壁打ちで以下を確定した。
...
2. **`deploy-layout.ts` の層化を 2 段に** —
   ...
   - DAG 配置の最終 Y の下に **job 帯**を 1 つのバンドとして配置する（同層内と同じ barycenter / `MAX_LAYER_WIDTH` サブロー折り返しを再利用）。
   - その下に従来どおり `__unclassified__` row。
3. **型** — `layout-types.ts` の `ContainerRect`（必要なら `LayoutNode`）に `kindBand?` を追加（optional、後方互換）。SVG で `data-kind-band="job"` を出せるようにする（将来の style/visual band 用フックだが、本 PR では layout 配置のみで可）。
4. **ghost edge** — 既存の edge-routing をそのまま使う。帯越しエッジが描けることをテストで担保。
5. **AT**: `docs/acceptance/1738-deploy-job-lane.md` を新規作成。TC 案:
...
6. **proactive TPL（実装 PR で同梱）** — 「kind ベースで一部 container を DAG から外して帯に移すとき、全ユニットが**ちょうど一度**配置される（drop / 重複なし）こと、帯をまたぐ ghost edge の端点が保たれること」を観点化する。`deriveInfraEdges` 共有（TPL-20260519-02）や scoped-glance（TPL-20260510-21）と関連付ける。
...
- **候補 B（store/infra 帯の正式コード化）は本設計では決めない** — store は現状 emergent に沈むので C で痛点は解消する。job 帯の運用実績を見てから B を別 Issue で評価する。
- **kind-band の視覚表現（背景帯・色・ラベル）は本 PR では決めない** — 本設計は layout 配置（Y バンド化）に限定。`data-kind-band` 属性は出すが、style-system 統合（#30）と視覚スイムレーン（候補 D）は将来課題。
- **balanced-grid（#1737）との適用順** — 帯は層分割（Y 軸）、balanced-grid は同層内格子化で直交。実装時に両者が同じ「同層内配置」関数を共有するなら、job 帯内の配置にも balanced-grid がそのまま効くことを確認する（競合しない想定）。
