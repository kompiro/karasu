...
5. **`.github/workflows/vscode-release.yml`**: `workflow_dispatch` トリガ。pnpm install →
   `pnpm --filter karasu-vscode build` → publish step は `if: VSCE_PAT が空でない` で gate。
   `VSCE_PAT` 不在時は `::notice::` を出して skip（release.yml の `NPM_TOKEN` guard を踏襲）。
   **初回は stable チャネルに publish する**（#1316 の "soft visibility" は「ひっそり stable
   publish」の意であり pre-release ではない）。ただし将来のために `workflow_dispatch` の
