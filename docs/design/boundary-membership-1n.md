...
- **ドキュメント**: `docs/spec/syntax.md`（+ja、boundary 節）、`docs/spec/diagnostics.md`（+ja）、`docs/roadmap.md` の `boundary` watch item（「first-wins 多重所属が実利用で噛み合うか」という観測項目は本件の決定で解消されるので記述を更新）。
...
- **team 軸（`ownerIndex`）の 1:N 化**: 構造は同型だが stable 構文であり、precedence に意味づけがある。本設計では決めない。新規 [TPL-20260729-01](../test-perspectives/TPL-20260729-01-declared-membership-not-discarded-in-derived-index.md) が再訪点を保持する。
