...
- **tip の数字は「その分岐点と共有端の間を spine が運ぶ本数」**。fan-in では共有端が target 側なので合流後の本数、fan-out では共有端が source 側なので分岐点までに届いた本数になる。規則は 1 つで、N 本の fan-out は source に近い分岐から N, N-1, …, 2 と並ぶ。最も遠い分岐は spine の端（L 字）なので数字を置かない。
- **spine の端（head）は共有端をはさんだ腕ごとに取る**。スライス A までの判定は「y が最小の stub が head」で、下向きの fan-in にしか正しくなかった。target が source より上にある fan-in では、ただの角に「1」を置き、target に最も近い合流を落としていた。fan-out は後退エッジで source の上にも枝を持ちうるので、この判定を「共有端から最も遠い elbow を腕ごとに head とする」に一般化した。下向きの fan-in では出力は変わらない。
- **fan-out の lane は spine の範囲が短いものから内側に置く**。枝は spine からカード側へ伸びるので、自分より内側にあり、その行を範囲に含む spine をすべて横切る。範囲が別の spine に含まれる spine を外側に置くと、そのすべての枝が内側の spine を横切る。boundary-clusters（examples）ではこの順で hop が 2 → 1 になり、上端の source から順に置く fan-in の lane 順のままだと 2 → 3 に増えた。fan-in 側の lane 順は幾何を変えない方針なので触らない。
