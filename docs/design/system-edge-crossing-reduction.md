...
### 2. 個別の external を最下段へ戻す style ヒント

作者が特定の external をサイドではなく最下段に置けるよう、ノード単位のヒントを設ける（例 `.krs.style` で `service#Stripe { placement: bottom }`、既存 `column: left/center/right` と同系統のプロパティ）。デフォルト sides・個別 opt-out。
...
1. 配置 post-pass: `assignForcedSystemLayers` の結果に対し、`systemTier===4`（external service）のノードを左右サイド列へ再配置（`computeLayoutEdges` の前に行い、エッジ再アンカーを自動化）。左右振り分けは declaration order / 既存 x を基準に半々。
...
- **style ヒント文法**: `placement: bottom` か、`column` 系を拡張するか。`[external]` は元々 tier 固定なので既存 `column` ヒントとの相互作用を確認。
