...
- `SvgIconDef` に `builtIn?: boolean` フラグを追加
...
2. **シェイプモードとの共存時のレイアウト密度** — アイコンモード（160×100 固定）とシェイプモード（可変サイズ）でダイアグラム全体の密度が変わる。モード切り替え時に全体再レイアウトを許容するか。

3. **コンテナノード（system）の扱い** — system ノードはカード型ではなくコンテナとして描画されるが、アイコンモードでの外観を別途定義する必要があるか。

4. **AI サマライズ** — 文字数超過時の `...` 打ち切りから、将来的に Claude API を使ったサマライズへ移行する設計（別 Issue）。
