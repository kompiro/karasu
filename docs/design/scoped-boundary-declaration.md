...
- **関連**:
  ...
  - 関連 Issue: [#2032](https://github.com/kompiro/karasu/issues/2032)（cross-file contains の偽 not-found）、[#2076](https://github.com/kompiro/karasu/issues/2076) / PR [#2086](https://github.com/kompiro/karasu/pull/2086)（`fmt` が top-level `boundary` を落とす — 本設計の前提修正）
...
| **formatter** | **`boundary` を 1 箇所も扱っていない** — `printFile` の emit list（`formatter.ts:113-120`）に `file.boundaries` が無く、`fmt` が **top-level boundary を黙って消す**。修正 PR [#2086](https://github.com/kompiro/karasu/pull/2086) が open | 同左 |
...
5. **formatter（`fmt`）**
   - 前提として **PR [#2086](https://github.com/kompiro/karasu/pull/2086) のマージが必要** — 現状 `printFile` の emit list（`formatter.ts:113-120`）に
     `file.boundaries` が無く、`fmt` は top-level boundary を**黙って消す**（`formatter.ts` に `boundary` 0 hit）。
   - #2086 が追加する `renderBoundaryBlock` を、**ノードブロックのレンダリング経路からも呼ぶ**必要がある。
   - **注意**: #2086 の網羅性ガード（`formatter-top-level-coverage.test.ts`）は `KrsFile` の配列プロパティ由来なので
...
- **examples**: 1 本追加。
- **他 PR との順序**: PR [#2086](https://github.com/kompiro/karasu/pull/2086)（fmt の top-level boundary 落ち）**の後**に着手する。
...
- [TPL-20260510-02](../test-perspectives/TPL-20260510-02-round-trip-guarantee.md) — round-trip 保証。
  **今回特に効く** — top-level boundary が `fmt` で消える bug（[#2076](https://github.com/kompiro/karasu/issues/2076)）が
  現に起きており、PR [#2086](https://github.com/kompiro/karasu/pull/2086) の網羅性ガードは `KrsFile` の配列プロパティ由来で
