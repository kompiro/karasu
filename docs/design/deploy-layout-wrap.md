...
- 未分類コンテナ（`realizes` なし）は最下段の独立行に置く。
  「依存グラフに属さない」ため、layer 0 に混在させるより意味的に正確。

## 将来の課題（sub-issue として登録済み）

- [#395](https://github.com/kompiro/karasu/issues/395) — 同レイヤー内のコンテナ順序（エッジ交差最小化）
  初期実装は挿入順。将来的に barycenter ヒューリスティックを適用する。

- [#396](https://github.com/kompiro/karasu/issues/396) — 孤立コンテナが多数ある場合の layer 0 の横展開
