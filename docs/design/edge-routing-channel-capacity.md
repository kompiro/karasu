...
| 貫通（dify 21 view） | 未計測 | **0 → 113 に悪化** | 5 → 5（不変） |
...
| 条件 | 測り方 |
| --- | --- |
| 貫通 0（柵の corpus） | `totalPenetrations(res) === 0`。対象は `routing-parity.test.ts` の in-repo モデル群（main で既に 0）と TPL-2598 の混雑 fixture。案2 が 0 → 113 に壊した側なので、水平の重なりと必ず同じテストで測る（TPL-1927） |
...
| 外部モデルで貫通を増やさない | 上の比較表の `5 → 5` はこの条件の実測。dify は repo に入らないので柵にできないが、既存 5 件（main 時点、本スライスの対象外）を増やさないことを実測で確認する |
