...
3. **`karasu`（CLI）を publish 可能にする**:
   ...
   - `packages/cli/package.json` に `publishConfig: { "access": "public", "provenance": true }`、`repository`、`homepage`、`files: ["dist"]`、`engines: { node: ">=20" }` を追加。`version` を `0.0.0` → `0.1.0` に。`src/index.ts` の `program.version("...")` も合わせる（package.json と同期する旨のコメントを付ける）。
...
3. **`karasu` の初期バージョン**: `0.0.0` → `0.1.0`（`packages/cli/package.json` と `src/index.ts` の両方）。`@karasu-tools/lsp` の `0.1.0` には手を付けない。
