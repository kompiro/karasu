...
- **日付**: 2026-06-28
- **種別**: PRD（product requirements / direction）。**ADR ではない** — 単一の architecture decision ではなく製品方針のため、ADR 昇格はせず **Issue に展開**する。
...
- 本書は **PRD として `docs/prd/` に残す**（ADR 昇格はしない）。要件は **Issue に展開**し、結論は `docs/roadmap.md` の keystone 節へ反映する。
  `docs/design/` は ADR 昇格前提の transient 置き場（昇格時に元ファイル削除）なので、ADR 化しない PRD はここには置かない。
