...
**投稿されたタイトルと `.krs` 本文は投稿者が書いたものなので、個人データを含みうる。** 受け取り口が
見るのは 2 つだけ — 構文として開けるか、資格情報の形をした文字列が無いか
（`packages/nest/src/gallery/validate.ts`）— で、氏名やメールアドレスを検出する規則は上の「個人データ
について正直に書く」と同じ理由で**存在しない**。担当者名・連絡先・社内の組織名がノードのラベルや
`description` に入ったまま届くことはありうる。**それを取り除くとは書かない。** 書けるのは
「入れるかどうかは投稿者が決める」「入っていても期限では消えない」までである。
