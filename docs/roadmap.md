...
| **M2** post-v1.0 notation watch | 柱 **Notation watch (round 2)**（[#1816](https://github.com/kompiro/karasu/issues/1816) ・`epic: notation-watch-r2`）。C/D/H/I は [§棚卸し finding の決着状況](#棚卸し-finding-の決着状況) と [§promotion gate](#promotion-gatenotation-評価の規律) で評価規律を規定 | ✅ #1816 **CLOSED**（2026-07-30、子 [#1818](https://github.com/kompiro/karasu/issues/1818)/[#1819](https://github.com/kompiro/karasu/issues/1819)/[#1820](https://github.com/kompiro/karasu/issues/1820) 完了）。継続 watch（finding 5/6）の正準台帳は [§finding 表](#notation-watch-round-2-の-finding) |
...
| --- | --- | --- | --- |
| **Notation watch (round 2)** | [#1816](https://github.com/kompiro/karasu/issues/1816) ・`epic: notation-watch-r2` | 実 OSS を書いて出た notation finding。[#1567](https://github.com/kompiro/karasu/issues/1567) の続編。既定 = experimental 据え置き | ✅ **完了**（#1816 CLOSED 2026-07-30、子 [#1818](https://github.com/kompiro/karasu/issues/1818)/[#1819](https://github.com/kompiro/karasu/issues/1819)/[#1820](https://github.com/kompiro/karasu/issues/1820) 済み）。残る watch（finding 5/6）は Issue でなく[下表](#notation-watch-round-2-の-finding)で追跡し、[§promotion gate](#promotion-gatenotation-評価の規律) で評価する |
...
| 5 | KV は **cache 用途**が多いが `database` は SoR 含意を持つ。`[index]` と並ぶ **`[cache]` role tag**（technology でなく role）が無い | **experimental 据え置き（watch）**。`--from wrangler` adapter は素の `database` へ degrade（warning）。cache パターンが複数 source で再発したら promotion gate へ | [ADR-1935](adr/1935-wrangler-translate-adapter.md)（Issue なし — 本表で追跡） |
| 6 | **Durable Object = stateful compute**（compute かつ store）で clean な infra kind が無い | **experimental 据え置き（watch）**。adapter は `service [external]` + RPC edge へ degrade（`[external]` は真の所有境界を過大表現）。honest な modeling 需要が実 corpus で溜まれば stateful-compute notation を再評価 | [ADR-1935](adr/1935-wrangler-translate-adapter.md)（Issue なし — 本表で追跡） |
...
>
> Epic [#1816](https://github.com/kompiro/karasu/issues/1816) は全子 Issue（#1818/#1819/#1820）の完了により 2026-07-30 に close した。以降、finding 5 / 6 の watch 台帳は**本表が正準**（Issue 側では追跡しない）。promotion trigger（[§promotion gate](#promotion-gatenotation-評価の規律)）を満たしたら、その時点で新規 Issue を起こして着手する。
