...
| --- | --- | --- | --- |
| **karasu-nest pivot** | [#1990](https://github.com/kompiro/karasu/issues/1990) ・`epic: karasu-nest` | GitHub App でリポジトリを読み、サーバ側 AI reverse で `.krs` を返す/描くホスト型サービス（[ADR-1990](adr/1990-karasu-nest-pivot-server-reverse.md)）。permalink との境界は [ADR-2249](adr/2249-permalink-generation-seam.md) で確定 | scaffold [#2227](https://github.com/kompiro/karasu/issues/2227) から PR-back delivery [#2289](https://github.com/kompiro/karasu/issues/2289) までの実装スタックがレビュー中。起点は intake の決定 [#2262](https://github.com/kompiro/karasu/issues/2262)（生成の起動権限と完了通知）で、これが決まるまでスタック全体が仮置き |
| **facets**（語彙の閉鎖） | [#2065](https://github.com/kompiro/karasu/issues/2065) ・`epic: facets` | ユーザー拡張点を `facet` に一本化し、tag / annotation はツール語彙へ閉じる（[§Syntax 2.0](#syntax-20-プログラム) の柱） | Part A と Part B の `facet` 構文・overlay は着地（[§watch 登録](#watch-対象の-experimental-notation登録)）。残るは style セレクタ [#2175](https://github.com/kompiro/karasu/issues/2175) と overview [#2177](https://github.com/kompiro/karasu/issues/2177) のレビュー |
...
| **`facet`**（要素横断の集合所属 — **唯一のユーザー拡張点**） | [#2160](https://github.com/kompiro/karasu/issues/2160)（[syntax](spec/syntax.md#cross-cutting-membership-facet--experimental)） | experimental。**昇格先は確定 — v2.0 core**（tags-and-facets 決定事項 4、[§Syntax 2.0 プログラム](#syntax-20-プログラム)）。宣言 + `facets` プロパティ + overlay は着地し、残るは style セレクタ [#2175](https://github.com/kompiro/karasu/issues/2175) と overview [#2177](https://github.com/kompiro/karasu/issues/2177) | 任意名 tag / annotation セレクタからの移行が実際に facet で置き換わるか（[§語彙の閉鎖](#語彙の閉鎖tag--annotation) の前提条件）。証拠源は同じく karasu-nest corpus |
...
| **`facet`**（外在的集合所属 — **唯一のユーザー拡張点**） | experimental（[§watch 登録](#watch-対象の-experimental-notation登録)） | **core**（v1.x に experimental で導入 → 実利用観測 → 昇格） | 導入 = [#2160](https://github.com/kompiro/karasu/issues/2160)。宣言 + `facets` プロパティ + overlay は着地し、残るは style セレクタ [#2175](https://github.com/kompiro/karasu/issues/2175) と overview [#2177](https://github.com/kompiro/karasu/issues/2177) |
