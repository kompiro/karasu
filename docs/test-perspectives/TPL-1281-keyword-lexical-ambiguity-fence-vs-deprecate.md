...
# TPL-1281: 既存キーワードの語彙的曖昧さは、まず外部 fence（別 ADR）で意味を縛れるか検討してから deprecate を判断する
...
- **外部 fence パターン**: 滑り台の起点となる構文（述語式・属性宣言・policy ブロック等）を別 ADR で「導入しない」と決め、spec の該当キーワードの定義にその ADR へのリンクを 1 行埋める。例: `user.role` の存続を決めた ADR-1281 は、authz 構文を取り込まない ADR-832 を外部 fence として参照することで成立している。
