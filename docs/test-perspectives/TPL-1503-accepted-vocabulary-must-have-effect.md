...
# TPL-1503: 受理される語彙は「効果を持つ」「警告される」「open set と明文化」のいずれかに属する
...
- [ ] そのエントリは spec（`docs/spec/*.md`）に記載されているか（TPL-1296 の同期チェック対象になるか）
...
- **スキーマ→consumer の片方向 subset チェック**: スキーマの全プロパティ名を列挙し、
  ...
  する smoke test。TPL-1296 の「正典→再掲の片方向チェック」と同型で、
