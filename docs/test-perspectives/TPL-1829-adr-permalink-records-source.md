...
# TPL-1829: ADR permalink は record ではなく pointer — in-repo .krs source を必須で記録する
...
- **記録は repo にある** — 構造の SoT は in-repo `.krs`（[TPL-1207](TPL-1207-text-as-single-source-of-truth.md)）。
...
- deep permalink の対象を `label` で指す。→ i18n / 表示文字列は identity ではなく、
  rename で別名に化ける（[TPL-2167](TPL-2167-id-not-label-for-identity.md)）。
...
- **記録は source・リンクは pointer**（[ADR-1829](../adr/1829-adr-permalink-convention.md)
