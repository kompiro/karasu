...
# TPL-1386: 新規 resolver diagnostic の register は「事実か流派判断か」で決める
...
- `packages/core/src/resolver/warnings.test.ts` — `domain-dispersal` が info で
  発火し、error 診断で描画をブロックしないことを確認するケース
- `packages/core/src/parser/parser.test.ts` — service 間で同名 domain id が重複しても
  parse error にならず nodePathIndex は先勝ちすることを確認するケース

## 派生元 spec

- `docs/concepts.md` §「What karasu visualizes vs. what it doesn't prescribe」
- `docs/concepts.ja.md` §「karasu が『描く』もの、『規定しない』もの」
- `docs/spec/syntax.md` §「Domain dispersal」 / `docs/spec/syntax.ja.md` §「ドメイン分散」
