...
# TPL-1738: 要素を主構造から抜き出して別グループに再配置するとき、全要素ちょうど一度配置 + 参照エッジの端点保持を検証する
...
2. **参照エッジの端点保持** — 抜き出した要素を端点に持つエッジ / 関係は、その要素が主構造の外に出ても **両端点が解決し描画される**（帯をまたいでルーティングされる）必要がある。フィルタの掛け方を誤ると、エッジ解決マップから端点が落ちて静かに消える。とくに再配置が **端点 id を書き換える**（複数要素を 1 つの stub / 代表ノードに畳む）変換のときは、主 edge リストだけでなく **同じ id を参照する全ての parallel edge リスト**を同一の remap に通さないと、書き換え漏れの端点が解決に失敗して代替アンカー（親コンテナ枠）に落ちる。
...
group-collapse（#1874）は逆に **id を意図的に書き換える**ケース: team を畳むとメンバーは `<Team> (N)` stub に統合され、`collapseGroups` が `childNodes` / `childEdges` の端点を stub id に remap する。しかし ViewSlice 上の ghost-edge リスト（`ghostSystemEdges` / `callerGhostSystemEdges` など）は別フィールドで、この remap を通っていなかった。結果、畳まれたメンバーを端点に持つ ghost コネクタは古いメンバー id を参照し続け、`computeLayoutEdges` の lookup に失敗して主コンテナ枠にフォールバック（アンカーがずれる）した。edge は消えないが端点が不正確になる劣化。
...
- 端点 id を書き換える畳み込み（fold → stub）で、主 edge リストだけ remap し、**別フィールドの parallel edge リスト**（view model 上の ghost / aux エッジ）を remap し忘れ、そのエッジの端点が古い id を参照し続けて **解決失敗 → 代替アンカー（親枠）にフォールバック**（アンカーずれ）。
...
- [ ] 再配置が端点 id を書き換える（fold → stub）なら、主 edge リストと **同じ id を参照する全ての parallel edge リスト**（view model 上の ghost / aux 系を含む）が同一 remap を通ることをテストする。畳まれた端点のエッジが stub にアンカーし、親コンテナ枠へフォールバックしないことを assert する。
...
- **id を書き換えるなら remap を 1 つの関数に集約して全 edge リストに適用する**: fold → stub のように端点 id を変える変換では、主 edge の remap 関数（`collapseGroups` 内の endpoint remap）と同じ写像を、view model 上の他の edge リスト（`ghostSystemEdges` 等）にも通す。remap を関数として `computeLayoutEdges` に渡し、ghost ループの端点 lookup / 出力 id 双方に適用する（group-collapse #1874）。
- **配置ロジックを 1 箇所に集約**: 主構造の行も別グループも同じ配置ヘルパー（`placeGroupBlock`）を通すことで、wrap / grid 規則の drift を防ぐ（[TPL-1415] と同趣旨）。
