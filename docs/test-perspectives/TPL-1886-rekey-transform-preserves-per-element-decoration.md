...
# TPL-1886: 端点 id を書き換える集約/畳み込み変換は、元 id にキーされた per-要素の装飾（diff state 等）を再導出する

## 観点

[TPL-1738](TPL-1738-relayout-into-group-preserves-placement-and-edges.md) は
...
TPL-1738 のチェックリストを全部通しても**すり抜ける**劣化。
...
- 装飾 map のキー形（`${from}->${to}`）とレイアウト id 形（stub id 由来）が**ずれ**、
  lookup が silent miss する（[TPL-1666] の「style lookup がレイアウト id 形と一致」を
  diff 装飾に一般化した形）。
...
端点 id を書き換える集約/畳み込み変換を実装/変更するとき（トポロジ柵 TPL-1738 に**加えて**）:
...
- [ ] 装飾 map のキー形が、レイアウト後の要素 id 形と**一致**することを確認する（[TPL-1666]）。
