...
# TPL-2170: edge / relation の片側が未解決でも、解決できた側のノードは drop しない
...
- [ ] 同じケースで **source / target のうち解決できた側のノードが残る** ことを直接 assert している（TPL-907 の「正しい参照に warning を出さない」と対称）
...
未確立（spec PR 後の実装 PR で追加予定）。`examples/ja/multi-file-system/` を使った AT でも「全ノードが現れること」を assert する。
