...
- **トリガを gate から path filter へ（構造的対処）**: app E2E は ADR-20260623-05 で `e2e` ラベル駆動を廃止し、`packages/app/**` / `packages/core/**` / `packages/e2e/**` 等の変更で自動起動する path filter に移行した。これにより app/core を触る PR では人間がラベルを覚えなくても E2E がマージ前に走り、本ブラインドスポットは構造的に塞がれる。先例は Preview workflow の同型移行（ADR-20260413-01）。
  - **未だ gated なスイート**（`vscode-e2e.yml` の `vscode-e2e` / `vscode-webview-e2e` ラベル）には同じ gap が残る。これらの surface を変える PR では下のリマインド手順を引き続き使う。
- **同 PR で grep → トリガ確認 → テスト更新**: surface を変える PR で `packages/e2e`（や `packages/vscode-e2e`）を変更セレクタで grep し、ヒットしたら対象スイートがその PR で起動することを確認（gated なら該当ラベルを付与）し、stale なアサーションを同 PR で直す。
...
- `.github/workflows/vscode-e2e.yml`（依然 label-gated。同型 gap が残る対象）
