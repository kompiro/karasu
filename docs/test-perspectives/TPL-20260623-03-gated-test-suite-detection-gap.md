...
- **同 PR で grep → ラベル付与 → テスト更新**: surface を変える PR で `packages/e2e` を変更セレクタで grep し、ヒットしたら `e2e` ラベルを付けて Playwright を回し、stale なアサーションを同 PR で直す。
...
- `.github/workflows/e2e.yml`（label gating の定義）
