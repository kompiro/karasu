...
# TPL-1415: 同一語彙を複数の表現で持つときは片方更新による静かな drift を検証する
...
- enum に値を足したが対応するラベルマップ / switch 分岐を足さず、`undefined`
  や fallback が表示される（[TPL-1094] の派生形）。
...
- `packages/core/src/builtins/default-style.test.ts` — builtin シートの
...
- [`docs/spec/i18n.md`](../spec/i18n.md) — core 節「組み込みアノテーションバッジのラベル」（ラベルの単一情報源は reference-data で、注入はそこからの生成で行う）
- [`docs/spec/tags-annotations.md`](../spec/tags-annotations.md) — shape タグ節（infra sub-resource kind `table` / `queue-item` / `bucket` と shape タグ `[table]` / `[queue]` / `[storage]` は同じ語彙の 2 表現。`INFRA_SUB_KIND_TO_TAG`（`packages/core/src/resolver/style-resolver.ts`）が dot 記法 resource の shape を infra kind から推論するため、両表現は整合し続ける必要がある）
- [`docs/spec/syntax.md`](../spec/syntax.md) — Infra layer 節（同上。infra leaf kind を追加・変更するときは shape タグ集合・shape registry・推論マップを同時に更新する）
