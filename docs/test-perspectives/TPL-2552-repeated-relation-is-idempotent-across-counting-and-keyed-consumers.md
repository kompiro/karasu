...
| **区別できる別々の事実**だが、**住所（id）が衝突している** | 報告する。そのうえで数える側を畳む側に合わせる | 同じ deploy ブロックの `oci app` が 2 つ（#2713） |
...
住所が衝突した 2 件をモデルに残すかは**層によって違う**、という点に注意する。1 ファイルを記録する
層（parser）は書かれたまま両方保つ。住所が一意な 1 つのブロックを組み立てる層（import の merge）は
1 件しか置けないので、報告したうえで落とす。karasu では単一ファイルの `deploy` が `app, app` を
保ち、merge 後のモデルは `app` 1 件になる。**報告は 1 つの誤りにつき 1 回**で、1 ファイル内で
見える衝突は parser が、merge でしか見えない衝突は merge が受け持つ。

どちらの層でも、数える側と畳む側の一致は宣言側では閉じない。だから**数える側が畳む側と同じキーで
数える**（グリッドは自分が keying するものを数える）。上流の producer 全員に一意性を保証させる形に
すると、次の producer が同じ食い違いを作り直す。
...
- [ ] 診断にした場合、**数える側も畳む側と同じキーで数える**ように直し、その診断が**その id を作りうる全経路**（単一ファイルの宣言・import の各 merge 経路・生成器）から、**1 つの誤りにつき 1 回だけ**出ることを assert したか
- [ ] **綴りが違って同じものを指す**書き方があるか（解決先が同じ 2 参照、カンマ列挙と行の繰り返し）。あるなら、そのすべてで結果が一致することを assert したか（resolve を持つ層の membership も冪等か — 宣言側の畳み込みだけでは届かない。TPL-2542）
- [ ] 扱いを spec に書き、兄弟プロパティと register が揃っているか（黙って畳む / 診断する のどちらかに揃っているか）を確認したか
...
- **報告して、数える側を畳む側に合わせる**: 同 deploy ブロックの重複 unit id は
  `duplicate-node-in-deploy`（error）で報告したうえで、`placeGroupBlock` が
  **自分が keying するキーで数える**（#2713）。黙って畳めない（2 件は別々の宣言）ので、
  一致は宣言側ではなく「数える側」の修正で取る
- **報告の担当を層で分ける**: `collectDeployNodeIds`（parser）が 1 ファイル内で見える衝突を、
  `mergeDeployNodes`（import）が merge でしか見えない衝突を受け持ち、merge は source 内で
  閉じた重複を畳むだけで再報告しない。両方が同じ衝突を報告すると、1 つの誤りが層の数だけ
  並ぶ（#2713 のレビューで実際に 2 件出た）
