...
# TPL-1225: 論理・物理・組織の三面は一つの `.krs` artifact 内で交差させる
...
- 三面のいずれかが optional / lazy になることで、`realizes` / `owns` の **cross-reference 検証**（→ TPL-907）が一部しか効かなくなる
...
- [ ] 外部システムとの統合は **取り込み（import / abstraction）** にとどめ、参照を外部 source に対する live link にしていないか（取り込み後は `.krs` が source of truth → TPL-1207 と整合）
...
- 関連 TPL: TPL-1207（text を single source of truth に保つ）/ TPL-907（cross-reference の resolver 側検証）
