...
- **到達範囲が説明できない** — 「bare は隣、dotted はどこでも」は、モデルが育つほど
  ...
  記法の軸で割り直される
- **上限の消失** — 接尾辞規則を無条件に入れると、別 system の内部構造へ任意の断片から
  直接エッジを張れてしまう。上限を与えるのは深さではなく anchor である: #2577 以前は
  ADR-104 の 2 セグメント記法がその役を兼ねていたが、深さの制限が外れた今、
  「トップレベル root から綴る」ことだけが到達範囲を構造に縛っている
