...
- spec の表だけ直して実装を直さず、「書いてあるのに効かない」規則になる
  （[TPL-20260610-01](TPL-20260610-01-accepted-vocabulary-must-have-effect.md)（受理された語彙は効果を持つ）の裏返し）。
- parser 側にだけ配置条件を足し、表に反映しないため Reference パネルと spec が
  古いままになる（[TPL-20260727-01](TPL-20260727-01-parser-acceptance-documented-in-spec.md)）。
