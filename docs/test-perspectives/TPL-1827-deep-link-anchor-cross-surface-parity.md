...
# TPL-1827: deep-link アンカーは id ベースの単一文法を全サーフェスで共有する
...
- アンカーの identity キーは要素の `id`（[TPL-2167](TPL-2167-id-not-label-for-identity.md)）であり、`label` や翻訳 / 表示文字列ではない。
- 文字列正規化（`sanitizeId`）と prefix 組み立ては**単一の関数**（`anchorId`）に集約し、producer が直書きの template literal で再現しない（[TPL-219](TPL-219-parallel-function-parity.md) の parallel parity）。
- 外部 URL から来る target 値は解決前に validate / canonicalize し、未知 view / rename 済み id は throw せず安全側（モデル全体 / view root）に degrade する（[TPL-168](TPL-168-trust-boundary-input-validation.md)）。
...
- [ ] 新しい `view`（`ActiveView` / `ShareTargetView`）を足したら両 producer・両パーサが網羅する（[TPL-1094](TPL-1094-enum-member-addition.md)）
