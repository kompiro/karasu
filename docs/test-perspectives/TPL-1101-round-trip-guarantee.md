...
- [ ] **同じ AST になる綴りが複数あるとき、出力テキストの綴りを pin したか**。AST 等価のテストはこの領域を見ない（#2650）。ドット記法の参照は `A.B.C` が引用符に化けないこと、bare / quoted の使い分けが著者の書いたままであることをテキストでアサートする
- [ ] **その綴りを復元するのに必要な情報が AST にあるか**。join 済みの文字列に畳んだ時点で境界は失われ、formatter 側の分解は推測になる（`KrsEdge.to` に対する `toPath`）。参照サイトを足すときはセグメントの形で持つ
