...
# TPL-2184: 同じモデリング状態を表す配置は同じ診断を出す
...
- **`canContain` に配置を足したときに検出器が置き去りになる** — 配置規則の正典は
  `canContain`（[TPL-2165](TPL-2165-containment-rule-has-single-definition.md)）だが、
...
- **文言と走査範囲がズレる** — 文言が親 kind の話（`not assigned to any service`）を
  ...
  読み手に判別できない。[TPL-1503](TPL-1503-accepted-vocabulary-must-have-effect.md)
