...
applicable_to:
  ...
  - "`docs/spec/glossary.md` のように、concepts / spec の定義を再掲する用語集・索引ページを追加 / 編集するとき（curated なので双方向 subset は縛らないが、各項目はリンク先の正典と矛盾してはならない）"
...
# TPL-1296: 人間向け spec ドキュメントと in-app reference データは片方向 smoke test で同期を縛る
...
これは #1234 / TPL-74 item 5 が parser に対してやっていることの reference データ版であり、TPL-219（並列関数ファミリの parameter parity）の「並列に存在するものは drift する」という観察を doc ↔ data の軸に広げたもの。
...
- 逆向き: parser / `reference.ts` 側に先にキーワードが landed したのに `docs/spec/*.md` に節がなく、`getReference()` / `ReferencePanel` 経由でしか辿れない状態が続く（infra-layer ノード種別 `database` / `queue` / `storage` / `table` / `queue-item` / `bucket` が ADR-316 以降ずっと spec doc 未記載だった #1327 で実際に発生）。`scripts/lint/krs-fences.ts` の `krs` fence チェックは構文の *使用例* は縛れても *節の存在* は縛れない点に注意。
...
- [ ] 逆向きも忘れない: parser / 文法（`LOGICAL_KEYWORDS`、infra ブロック、トークン種別等）や `reference.ts` 側にキーワード（ノード種別含む）を先に足したら、同じ PR で `docs/spec/syntax.md`（必要なら `style.md` / `tags-annotations.md`）にその layer・用途・制約を記述する節を足したか。`scripts/lint/krs-fences.ts` の `krs` fence は使用例しか縛れないので、節の追加は人手で確認する。
...
- `packages/core/src/builtins/reference-spec-sync.test.ts` — spec doc ↔ `getReference()` の片方向 subset チェック（本 TPL の主たる担保）。`PROPERTY_SCHEMAS ⊆ spec doc` の逆向き subset チェック（#1492）も同ファイル
- `scripts/lint/krs-fences.ts` — ドキュメントに埋まった `krs` fence ↔ parser（#1251 / #2415 / TPL-74 item 5）
