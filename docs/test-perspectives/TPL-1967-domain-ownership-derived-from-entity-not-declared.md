...
# TPL-1967: infra leaf のドメイン所有は entity 層から導出する（物理 table に宣言しない）

## 観点

infra leaf の「所有ドメイン」は **論理層（`entity` の親 domain）から導出される単一の事実**であり、
物理 `table` / `queue-item` / `bucket` には決して宣言させない。これを破ると論理/物理の分離が
崩れ（[TPL-1415](TPL-1415-shared-vocabulary-dual-representation.md) の dual-representation）、
...
4. **read/write の取り違え** — `mode` は read/write/readwrite を集約する。write-only 発火にしたり
   read を落としたりしない。severity は read/write とも info（[TPL-1386](TPL-1386-diagnostic-register-fact-vs-style.md)
   の事実 vs 流派判断）。
5. **解決集合の非同期** — resource→store の解決を `deriveInfraEdges` / `detectSharedInfraFanIn` /
   `detectUnassignedResources` と別ロジックで実装し drift させる（[TPL-1720](TPL-1720-validation-target-set-enumerates-all-kinds.md)）。
