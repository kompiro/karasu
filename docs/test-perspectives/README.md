...
`docs/concepts.ja.md` の **「非目標 → 共通フィルタ」**（karasu はゆっくり変化する構造的な文脈だけを扱う）は、特定の `topic` / `scope.packages` に紐付かず **すべての feature** に適用されるため、TPL ファイルではなく `CLAUDE.md` の TPL セクションに「メタフィルタ」として配置している。新 feature / 構文拡張 / 連携機能の提案・レビューでは、個別 TPL のチェックに加えてこのメタフィルタも必ず通すこと。配置の決定経緯と却下した代替案は #1221 を参照。
