...
| [TPL-2088](TPL-2088-unscoped-bare-id-reference-reports-multiplicity.md) | 宣言スコープを持たない bare-id 参照（`owns` / top-level `contains`）は多重解決を宣言順に依らず報告する — 判定は集合を作り終えてから行い、(kind, 深さ) が揃った衝突（移行共存・マルチテナント）は正当なので沈黙させる | parser | ADR-927, docs/concepts.ja.md, #2088 |
