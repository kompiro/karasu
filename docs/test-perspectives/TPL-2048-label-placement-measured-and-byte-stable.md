...
エッジルーティングの可読性を「交差数だけでなく貫通数も数値で測る」（[TPL-1927](TPL-1927-routing-measures-crossings-and-penetrations.md)）のと同型に、**エッジラベルの配置（衝突回避）も目視ではなく数値で計測して assert する**。ラベルは「線」ではなく「箱（bounding box）」なので、計測軸はエッジ幾何とは別に 2 つある:
