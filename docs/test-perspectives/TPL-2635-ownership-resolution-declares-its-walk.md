...
各項目は、そのコードに対して yes/no で答えられる:

1. `owns` を持たない子ノードを含む fixture に対し、遡らない読み手が「チームなし」を、
   遡る読み手が「祖先の team」を返す — **同じ fixture で両方が固定されている**
2. 子と祖先の両方に `owns` があるとき、遡る読み手が返すのは**子の team だけ**である
3. 同一ノードを 2 team が `owns` するとき、遡る読み手が**両方の team** を返す
4. 遡っても team に届かない端点が**出力に残っている**
5. `user` など所有しえない kind の端点が、その数え上げに**入っていない**
