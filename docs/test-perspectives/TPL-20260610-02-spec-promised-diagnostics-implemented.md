...
1. **診断が存在しない** — spec はメッセージ文面例つきで警告を約束しているのに、
   実装に検出コードがない。2026-06-10 の監査では `docs/spec/style.md`
   「@import scope and conflicts」が約束するセレクタ重複警告
   （"Selector ... is defined in multiple files"）が未実装だった
...
- [ ] spec に「警告 / エラーを出す」と書いた約束それぞれに、対応する専用診断コードが実装に存在するか（grep でメッセージ・コード名を引いて確認）
...
- `docs/spec/style.md` — 「@import scope and conflicts」節（セレクタ重複警告の約束）
