...
# TPL-1535: 自分で書き込みつつ監視するストアでは、書き込みを直列化し自己エコーと非同期 read レースを処理する
...
- editor 書き込みを `SerialQueue.run()`（`packages/app/src/fs/serial-queue.ts`、TPL-1530 と共用）で直列化 → ディスクは最新値で終わる
