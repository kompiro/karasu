...
# TPL-2167: identity 判定 / 比較 / 集約のキーには `id` を使い、`label` などの表示・翻訳文字列を使わない
...
- export / format の round-trip で `label` が変わると identity が壊れる（→ TPL-1101 round-trip と関連）
...
- export / formatter で `label` を変換しても `id` は変えない（→ TPL-1101 round-trip 保証 と整合）
