...
# TPL-1983: view-state オプションの適用範囲制限（gate）は全 render surface で同一条件に揃える

## 観点

[TPL-219]（parallel-function-parity）は「新しいオプションを全 call site に**通す**」
...
- 「オプション × surface」のマトリクスを列挙するメタテスト（[TPL-1001] が
