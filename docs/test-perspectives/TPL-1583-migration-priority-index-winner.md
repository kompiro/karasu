...
# TPL-1583: 1:1 index は migration 共存の重複から @migration_target を勝者に選ぶ — 全 index で一貫させる

## 観点

karasu は逆コンウェイ戦略の過渡期（移行元 → 移行先の引き継ぎ中）に、同じものが
2 箇所に属する状態を **事実として描く**（[[TPL-1386]]）。一方、内部の
索引は 1:1 のものが複数ある:
...
- **priority のハードコード重複**: `migration_target ? 2 : deprecated ? 0 : 1` の
  ...
  片方だけ更新され静かに drift する（[[TPL-1415]] と同型の dual-representation drift）。
...
- [ ] 重複が起きても info 診断（`duplicate-owner-assignment` / `domain-dispersal`）は
      引き続き発火し、severity が error に戻っていないか（[[TPL-1386]]）
