...
# TPL-1680: AT ↔ e2e spec の linkage は machine guard で固定する

## 観点

`docs/acceptance/*.md`（AT）と `packages/e2e/tests/at-*.spec.ts`（e2e spec）は、`> ✅ Automated by \`<spec path>\`` という marker で結ばれている。これは「ある AC を自動で守っているのはどの spec か」を **AT 側から辿れる** ようにするための紐付けであり、TPL-1716（app/CLI surface ↔ docs/tools）と同じ「同じ意図を 2 つの表現で持つ」構造の **テスト軸版** にあたる。
