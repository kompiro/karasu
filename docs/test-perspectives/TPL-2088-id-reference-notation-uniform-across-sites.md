...
- **受理はするが同じ形で書き戻せない** — 記法を揃えても、AST がセグメントを join した
  文字列で持つサイトは `karasu fmt` が path 全体を 1 個の文字列リテラルに書き換える
  （#2650 の edge endpoint）。author から見れば「その形は通るが `fmt` をかけると
  別の綴りになる」であり、受理が半分しか本当でないのは分岐と同じである。AST 等価は
  保たれるので round-trip テストは沈黙する（[[TPL-1101]]）
...
- [ ] 受理した形を**そのまま書き戻せる**か。AST がセグメントの形で持ち、`karasu fmt` の
      出力が著者の綴りと一致することをテキストでアサートしたか（[[TPL-1101]]、#2650）
