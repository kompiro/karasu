...
# TPL-20260716-01: keystone・permalink の coined 用語は単一の正典を持つ
...
**他ドキュメントは定義を再掲せず用語集を参照する**ことで防ぐ。TPL-20260511-02 が
...
2. **用語集は機構ドキュメントと矛盾しない**。permalink family の各項目は
   ...
   定義がリンク先と食い違わないようにする（TPL-20260511-02 の「再掲は正典と矛盾しない」）。
...
in-site リンク（published ページ間）は `check-links`（TPL-20260616-01）が解決を縛るが、
