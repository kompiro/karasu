...
- 逆に、**どんな色でも届かない fade 状態**を判定面に入れてしまう。枠線に置ける
  最良の色は純白だが、dark canvas に対して facet dim (0.28) で 2.50:1、
  diff ghost (0.3) で 2.70:1 にしかならず、色の選び直しでは解けない。ここは
  WCAG 1.4.11 の inactive component 除外に倣って対象外と**書く**（黙って外すと、
  次の人が「測り忘れ」と区別できない）。
