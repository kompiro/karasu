...
# TPL-1927: エッジルーティング/レイアウトの可読性を検証するときは、交差数だけでなくノード/フレーム貫通数も測る（貫通は 0 を assert）
...
この観点は [TPL-1736](TPL-1736-tier-split-no-edge-penetration.md)（ティア分割で中間カードを貫通しない）を **2 点で拡張**する:
...
- [TPL-1736](TPL-1736-tier-split-no-edge-penetration.md) — ティア分割で中間カードを貫通しない（本 TPL が frame + 二重計測へ拡張）
