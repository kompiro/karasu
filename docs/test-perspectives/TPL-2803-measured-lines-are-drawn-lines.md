...
- [ ] 経路ごとに、**その経路が描いた行集合が、その経路の測定が確保した行集合と一致する**テストがある
      （例: shape mode で `data-meta-glyph` / `data-client-capability-count` の数をシートの有無で比べる）。
      比べるのは同じ測定を共有する範囲に限る。`renderDefaultText` は description の折り返し幅を
      表示モードで変えるので、モードをまたいだ行数の一致は要求にならない。
