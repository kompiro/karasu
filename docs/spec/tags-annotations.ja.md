...
- **印が無い = 確認済み、ではない。** karasu はレビュー状態を追跡しない。`@draft` の無いノードは単に誰も印を付けなかったノードであり、手書きモデルではそれが通常である。生成モデルでは、生成器が付けた印だけがそのモデル自身についての唯一の信号だと考える。**どこにも `@draft` の無い生成モデルは強い主張をしている**ので、疑ってよい。
