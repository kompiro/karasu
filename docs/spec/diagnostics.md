...
| `invalid-owns` | warning | An `owns` target **resolves to a node** whose kind cannot be owned. An id that resolves to nothing is not this diagnostic's business — `owns-target-not-found` reports that, so one mistake draws one code. Import-coupled as a consequence: in a single-document context a cross-file target resolves to nothing, so nothing is reported. The ownable kinds are `service` / `domain` / `client` / infra blocks at any depth (`OWNS_TARGET_KINDS`, the one enumeration this and `owns-target-not-found` share); an infra leaf (`table` / `queue-item` / `bucket`) and a `capability` exist but are not ownership units, so this is the check that rejects them. Note the system-view **card** draws a team chip only for the logical kinds; an owned infra block still reads under *Group by: team* (frames resolve by id) and in the org view. |
