...
- **A value is one string literal or one bare word**: anything else is an error (`annotation-param-value-unreadable`) and nothing is recorded. That covers a word starting with a digit (`until: 2026`), a keyword (`from: system`), and a hyphenated or dotted run (`until: 2026-12-31`, `from: Legacy-Monolith`, `from: Shop.Legacy`). Quote such a value: `until: "2026-12-31"`, `from: "Shop.Legacy"`. It is an error rather than a warning because `karasu fmt` refuses a file with errors; a warning would let `fmt --write` print the bare annotation and delete the value the author wrote. The diagram still renders.
- **One value per parameter on an element**: writing the same annotation twice on one element warns (`duplicate-annotation`). Giving the same parameter two different values, across repeated annotations or inside one, is an error (`annotation-param-conflict`); the first value is kept. An element holds one value per annotation and key, so `fmt` could otherwise only print one of the two over the other.
...
`Legacy`'s value is not one token (write `until: "2026-12-31"`), and `Billing` gives `until` two different values.
...
>
> `[projected]` is in the same register as `[implicit]` — assigned at render time, never present in `.krs` source — and is its mirror image: `[implicit]` rolls domain edges **up** to the service level, `[projected]` rolls entity relations **down** onto the `table` leaves they map to. An untagged edge on the same canvas is one the `.krs` records (written by hand, or emitted from a declared foreign key), so untagged reads as **confirmed** and `[projected]` as **asserted by the entity layer**. That axis is who confirmed the relation, not which tool wrote the line.
