...
- **Structural overlap** — a *derived* fact about containment rather than about
  calls: a node one team owns living inside a node another team owns. **Both
  ends must declare `owns` themselves** — an inherited owner is by definition
  ...
  dependencies, in its own channel, and marked `nested` when one team is the
  other's ancestor in the org tree.
