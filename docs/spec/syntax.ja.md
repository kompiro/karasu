...
| キーワード | 意味 | 含むことができるもの |
|-----------|------|-------------------|
| `system` | owned/external なサービスやクライアントの関係を示す器 | `service`, `user`, `client` |
...
| `client` | エンドユーザーの代わりに動作する、プロジェクトが配布するソフトウェア（mobile / web / desktop / CLI / device / extension / embed）。認識される form-factor タグは下表参照 | — |
