...
**差分は描くだけでなく報告される。** `karasu coverage` は `database` ごとに両集合を突き合わせ、
順序付き `{from, to}` leaf ペアの一覧を 4 つ報告する（`unmappedButReferenced` と同じ形で、
...
弾かれた宣言はブロックの残りを解析するために AST に保持されるが、**どのビューにも
描画されない**。信号はエラーだけであり、起点となるブロックへエッジを移して初めて
描画される。描画してしまうと 1 つの関係に 2 つの綴りを与えることになる:
`service S1 { S2 -> S3 }` は、規則が本来求める形（`S2` の中に書いた `S2 -> S3`）と
まったく同じ絵になる。
