...
| `annotation-param-value-unreadable` | error | 認識される annotation パラメータの値が、文字列リテラル 1 つでも裸の語 1 つでもない（`until: 2026-12-31`、`from: system`、`from: Shop.Legacy`）。何も記録しない。`karasu fmt` が裸のアノテーションを出力して値を消すのではなくファイルを書き換えないよう、エラーにしている。 |
| `annotation-param-conflict` | error | 1 つの要素が同じ annotation パラメータに異なる 2 つの値を与えている（アノテーションを繰り返した場合も、1 つの中で繰り返した場合も）。最初の値を保つ。 |
