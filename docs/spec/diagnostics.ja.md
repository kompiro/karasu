...
- **`file` が無いときは、消費者自身の文書として読む。** これが生じるのは単一文書の
  文脈だけである。LSP は開いている文書を 1 つずつ parse し、`karasu lint-style` も
  同様である。
...
| app のプレビューバナー | 開いている文書の位置、または `file` の無い位置は `Line <line>`。それ以外のファイルは `<path>:<line>` で、プロジェクトルート配下ならそこからの相対パス。 |
