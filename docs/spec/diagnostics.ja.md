...
| app のプレビューバナーと warning パネル | 開いている文書の位置、または `file` の無い位置は `Line <line>`。それ以外のファイルは `<path>:<line>`。パスはプロジェクトルートからの相対パスで、プロジェクトの無いモードではエントリのディレクトリからの相対パスになる。比較中のスナップショットは、撮影元のプロジェクト上のパスで示す。 |
