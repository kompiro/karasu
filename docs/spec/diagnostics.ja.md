...
| `facet-not-declared` | warning | `facets` の参照先の `facet` ブロックが宣言されていない（存在検証はマージ後のモデルで行うので、import 先の宣言も有効）。near-miss の annotation ヒントと違い、宣言集合が「正」を与えるためこの検査は完全で、著者定義の名前どうしの取り違えも検出する。プロパティが書かれた場所で報告し、ノードなら node id、エッジなら（エッジ自身の id が無いので）canonical base 形（`A-->B`）で名指す。これは `edge#<id>` が同じエッジを指すのと同じ綴り。 |
