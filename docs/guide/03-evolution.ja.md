...
ノードに `@<アノテーション>` を付けて、ライフサイクル上の状態を宣言します。移行を語るのは次の 5 つです。
...
| `@planned` | 設計上そこに置くが、まだ実在しない | レビューで合意しただけでコードは無いサービス |

「**まだ無い**」を言えるのは `@planned` だけです。他の 4 つはいずれも実在を前提にしています。to-be の姿を描いて、そのうちどこがまだ計画なのかを印す — 設計判断はふつうそうやって記録されます。（6 つ目の `@draft` は移行ではなく確度についてのアノテーションで、誰も確認していない記述に付けます。[タグ・アノテーションのリファレンス](../spec/tags-annotations.ja.md#draft--主張されているが未確認)を参照。）
