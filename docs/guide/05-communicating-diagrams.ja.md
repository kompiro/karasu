...
[境界設計ガイド §2](01-service-team-design.ja.md#2-逆コンウェイ戦略--アーキテクチャに合わせてチームを設計する) で `owns` によりオーナーを **構造として** 記録したのと対に、色は同じオーナーシップを **視覚として** 伝えます。完全例は [`examples/payment-platform/`](../../examples/payment-platform/)（`theme.krs.style` を `@import`）。
