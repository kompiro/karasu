...
>
> 📚 ガイドシリーズ 第5章 / 全5章 ｜ ← 前章: [アクセス経路とクライアント](04-access-paths.ja.md)
...
他のガイド（[境界設計](01-service-team-design.ja.md) / [オンボーディング](02-onboarding.ja.md) / [進化](03-evolution.ja.md)）が「何をモデルに書くか」なら、こちらは「書いたモデルをどう伝えるか」です。
...
[境界設計ガイド §2](01-service-team-design.ja.md#2-逆コンウェイ戦略--アーキテクチャに合わせてチームを設計する) で `owns` によりオーナーを **構造として** 記録したのと対に、色は同じオーナーシップを **視覚として** 伝えます。完全例は [`examples/ja/payment-platform/`](../../examples/ja/payment-platform/)（`theme.krs.style` を `@import`）。

---

## 3. ライフサイクル状態を色・バッジで示す

[進化ガイド](03-evolution.ja.md) のライフサイクルアノテーション（`@deprecated` 等）は、スタイルセレクタで見た目に反映できます。`opacity` で薄く、`badge-label` / `badge-icon` / `badge-color` でバッジを付けて、移行の状態を図上で一目化します。
...
アノテーションは親から子へ継承されるため（[進化ガイド §2](03-evolution.ja.md#2-アノテーションの継承--drill-down-しても文脈を失わない)）、`@deprecated` な service をドリルダウンした配下のノードにも同じスタイルが効きます。
...
凡例ラベルは著者が直接書く文字列で、i18n の対象外です。サンプルは [`examples/en/feature-samples/legend.krs`](../../examples/en/feature-samples/legend.krs)。
...
PR で図の diff を見せたいときは [進化ガイド §4](03-evolution.ja.md#4-karasu-diff--アーキテクチャ変更を可視化する) の `karasu diff` を併用します。
...
- 関連ガイド: [境界設計](01-service-team-design.ja.md) / [オンボーディング](02-onboarding.ja.md) / [進化](03-evolution.ja.md)
...
- チーム別カラーの例: [`examples/ja/payment-platform/`](../../examples/ja/payment-platform/)
