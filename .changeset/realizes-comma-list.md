...
A malformed list (`realizes A,` / `realizes ,B`) now reports a single `expected-property-value` on the offending comma, instead of the generic `unexpected-token-in-block` on whatever followed. `unresolved-realizes` is likewise reported on the target identifier that failed to resolve, spanning just that identifier, rather than on the whole deploy node.
