...
| `handles-target-ambiguous` | warning | A `handles` reference resolves to two or more declared `domain`s at different depths (the kind is fixed, so the discriminator reduces to depth); same-depth duplicates are the multi-tenant broadcast pattern and stay silent. Existence and the one-hop expose rule stay with `unresolved-handles`. Import-coupled: decided only on the merged model. |
