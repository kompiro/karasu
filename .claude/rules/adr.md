...
**正は自己完結した `/s?s=<payload>` URL**（凍結スナップショット）。短縮 URL
（taka 等）は任意の差し替え可能な別名で、**正にしない＝必須依存にしない**
（[TPL-20260630-02](../../docs/test-perspectives/TPL-20260630-02-adr-permalink-payload-is-canonical.md)）。

### Frontmatter `permalink:`

frontmatter を正とする。`payload` が必須、`short` / `source` / `view` は任意:

```yaml
permalink:
  - payload: https://karasu.kompiro.dev/s?s=<self-contained payload>  # 必須・正（凍結・immutable）
    short:   https://taka.kompiro.dev/TkrZQG                          # 任意: 表示用別名（任意 shortener）
    source:  examples/payments/system.krs                             # 任意: living source への traceability
    view:    system                                                   # 任意: 既定 view
```

- `payload` は **`/s?s=`（query）形のみ**。`#s=`（fragment）は server に届かず
  unfurl が死ぬので不可（ADR-20260626-04）。
- `short` を載せる場合も宛先は `/s?s=`。短縮しない選択も可。
...
`permalink:` の検証（`payload` が decode 可能な有効 `/s?s=` か・任意 `source` の
`.krs` が実在するか・任意 `short` が 302 で `payload` に解決するか）と本文サマリ
生成は [#1830](https://github.com/kompiro/karasu/issues/1830) / `@kompiro/adr-tools`
に委ねる。現状の `adr:validate` はまだ `permalink:` を検証しないため、当面は手書きで
規約に従う（スキーマ未対応の段階で実 ADR に貼ると検証が付かない点に留意）。
